...
Thus the objective of this experiment is to study if the removal of the previously identified markers have an impact on the perceived pleasantness. Two hypothesis are formulated:
...
\begin{itemize}
\item \emph{Church bell}, \emph{bicycle bell}, \emph{animals} for the i/sm-scenes;
\item \emph{siren}, \emph{car horn} for the ni/sm-scenes.
\end{itemize}
...
The subjects evaluate the 144 scenes. The evaluation is done on an 11-point bipolar semantic scale ranging from -5 (non-ideal / very unpleasant) to +5 (ideal / very pleasant). Before grading a scene, subjects must listen to the first 20 seconds. After scoring, they are free to move on to the next scene.

For each subject, the scenes are presented in random order. The first 10 scenes allow the subject to calibrate his/her grades. Those calibration scenes are  5 i/km-scenes and 5 ni/km-scenes. These first 10 scenes are replayed at the end of the experiment, and only the notes given at the second occurrence are taken into account.
...
These results tend to confirm that the perception of the qualities of a scene depends above all on the sound sources that compose it, the compostional characteristics taken into account during the perceptual process appearing to vary from one source to the other, from one type of environment to the other. This fact lead the authors to conclude that there is little hope to find a holistic physical descriptor that can adequately account for the affective qualities of all types of sound environment.
...
\begin{itemize}
\item in the case of non ideal scenes, one such focus on reducing the acoustic pressure level, whether globally, or by discarding specific sources such as \emph{sirens} or \emph{car horns}
...
Third, the fact that the compositional descriptors correlated with the perceived pleasantness change according to the type of scenes leads us to believe that the compositional characteristics allowing to distinguish two instances of the same abstract concept linked to the pleasantness are not the same for EA and ED.
