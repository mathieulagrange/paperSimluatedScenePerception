...
All the subjects perform the experiment simultaneously. They were not allowed to talk to each other during the experiment.
...
To conclude, the level of ni- markers has a negative influence on the pleasantness for the non-ideal scenes. On the other hand, the level of the ideal markers does not influence the perceived pleasantness for the ideal scenes.
...
The aim here is to validate the hypothesis that the removal of ideal markers and non-ideal markers has a significant effect on the perceived pleasantness. To do so, we perform an analysis of variance (ANOVA). We consider the pleasantness rating of each subject on each scene as a dependent variable, and as independent variables, the type of environment (ideal or non-ideal) and the presence/absence of markers. As each subject evaluated the whole set of scenes, a two factors repeated measure ANOVA with interaction is used to evaluate whether there exist a significant difference of perceived pleasantness. The two independent variables are considered as \emph{within-subject} factors. The factors being of only two levels each (type: ideal or non-ideal,  marker: with or without), the sphericity hypothesis does not need to be checked. \emph{Post hoc} analyses are done using the Tukey-Kramer procedure. %\jfp{l'analyse est inutile a mon avis, il n'y a que 2 niveaux pour chaque facteur de l'ANOVA.} \gl{2 niveaux par facteur, mais 4 niveaux en tout}
...
In this section, we study the scores given by the subject while listening to the several types of scenes, namely the ideal scenes with markers, non-ideal scenes with markers, ideal scenes without markers and non-ideal scenes without markers (Figure~\ref{fig:xp2_Ascene}). The repeated measure ANOVA applied to the pleasantness rating shows a significant effect of the type environment (Type: $F[1,10]=175$, $p<0.01$), of the presence/absence of markers (marker: $F[1,10]=7$, $p<0.05$), and of the interaction between those two factors ($F[1,10]=67$, $p<0.01$).
