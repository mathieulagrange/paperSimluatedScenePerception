...
\begin{frontmatter}
 ...
 cognitive psychology \sep soundscape perception \ soundscape simulator
...
44 students (14 females; averaging 21.6 years of age, s.d. of 2.0 years) from \emph{Ecole Centrale de Nantes} (a french engineering school) took part to the experiment. They are all living in downtown Nantes (a french city) since 2 years or more.
...
10 students (2 females; averaging 23.1 years of age, s.d. of 1.8 years) from \emph{Ecole Centrale de Nantes} (a french engineering school) took part to the experiment. They are living in downtown Nantes (a french city) since 2 years or more. None of them took part to the previous simulation experiment (Exp. 1.a).
...
\item \emph{analysis of the semantic features}: the goal is to evaluate if specific classes are more frequently used to simulate a given type of environment (i or ni). A V-test is considered \citep{lebart2000statistique} \ml{y pas une ref en Anglais ?}. With $c$ being the total number of classes used for both types of environment, $c_k$ the number of classes used for a given type of scene $k$ ($k=\lbrace i,ni\rbrace$), $c_j$ the number of time a class $j$ has been used, and $c_{jk}$ the number of time a class $j$ has been used for a given type of scene $k$, the V-test tests the null hypothesis that the ratio $\frac{c_{jk}}{c}$ is not significatively different from the ratio $\frac{c_{jk}}{c_k}$. For each class $j$, and each environment type $k$, an approximation of the statistic value $V_{jk}$ is computed as follows: 

\begin{equation}
V_{jk}=\dfrac{c_{jk}-c_k\frac{c_j}{c}}{\sqrt{c_k\frac{c-c_k}{c-1}\frac{c_j}{c}(1-\frac{c_j}{c})}}
\end{equation}

If the hypothesis is rejected, the class $j$ is said to be typical with respect to the type of scene $k$. The typical classes are called \textbf{sound markers}. Testing is done for each class, at every level of abstraction, and separately for texture and event classes;
...
%\jfp{on se demande pourquoi on va de -5 a +5 alors qu'avant on allait de -3 à +3}
...
%\jfp{je ne comprends pas les 2 p-value. il ne peut y a voir qu'une seule p-value lorsqu'on compare deux conditions expérimentales}
