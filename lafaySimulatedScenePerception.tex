...
All the participants start by designing the ideal environment; they read the second set of instructions at the end of the first experiment. Participants are completely free of their choices concerning sounds and synthesis parameters. The created sound environments must nevertheless fullfill the two following constraints :
...
In addition, once the two sound scenes are completed, participants are invited to:
...
Before starting the first simulation, a 20-minute tutorial is given to the participants in order to familiarize them with the simulation interface and the sound database. The experiment is planned to last about two hours and a half, including breaks that the subjects are allowed to take.
...
All the participants perform the experiment on standard desktop computers with the same hardware and software configurations. The audio files are played in diotic conditions using headphones. During the tutorial, participants are asked to adjust the sound volume to a comfortable level. Once set, they are not allowed to modify it during the remaining of the experiment.
...
All the participants performed the experiment at the same time. They are equally distributed in three identical quiet rooms. They are not allowed to talk to each other during the experiment.
...
Three experimenters (one in each room) are available during the whole duration of the experiment in order to assist participants with potential hardware and software issues and to answer questions.
...
The assessment is done with a 7-point bipolar semantic scale going from -3 (non ideal / unpleasant) to +3 (ideal / pleasant). Before evaluating a scene, participants must listen to the first 20 seconds of the stimuli. After the evaluation, they are free to continue to the next scene.
...
For each participant, sound scenes are played in a quasi random order. 5 ideal scenes and 5 non ideal scenes are first sequenced to allow the participants to calibrate their scores. These first 10 scenes are played back again at the end of the experiment. Only the last evaluations are taken into account.
...
All the participants perfrom the experiment simultaneously. They are not allowed to talk to each other during the experiment.
...
An experimenter is available during the whole duration of the experiment in order to assist participants and answer questions any questions the participants may have.
