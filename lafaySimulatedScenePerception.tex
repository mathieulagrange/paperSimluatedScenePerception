...
Ce Papier présente les résultats d'une série d'expériences s'appuyant sur la simulation, et visant, chacune, à comprendre comment les différentes sources sonores qui composent une scène influent sur la perception de l'agrément:

\begin{enumerate}
\item \emph{expérience 1.a, simulation}: au cours de cette expérience, les sujets simulent les environnements qui serviront de stimuli pour les étapes suivantes;
...
Le papier comprend 5 sections. La première constitue notre introduction. La deuxième introduit l'outil de simulation \emph{SimScene}. La troisième reprend l'expérience de simulation à proprement parler, ainsi que l'épreuve \emph{évaluation de l'agrément}. La quatrième décrit l'épreuve \emph{évaluation de l'agrément après modification des scènes}. Nous concluons dans la dernière partie. 

Précisons que l'expérience de simulation a fait l'objet d'une étude pilote dont les résultats sont présentés dans  \cite{lafay2013atiam,lafay2014new}. L'outil de simulation \emph{Simscene} est présenté quant à lui en détails dans \cite{rossignol2015simscene,lafay2016JAES}. Nous résumons ici ses fonctionnalités d'importance pour notre étude.
...
\emph{Simscene} est un environnement de travail audio-numérique dont la première version a été développée dans le cadre du projet HOULE\footnote{Projet HOULE : \url{http://houle.ircam.fr/}}. Il permet de simuler des paysages sonores à partir d'un corpus de sons. Il est prévu pour fonctionner sur les navigateurs internet \emph{Chrome} et \emph{Firefox}. L'outil a été développé en javascript à l'aide de la bibliothèque \emph{angular.js}\footnote{\emph{angular.js} : \url{https://angularjs.org/}} et du standard \emph{web-audio}\footnote{\emph{web-audio} : \url{http://www.w3.org/TR/webaudio/}}. L'interface de sélection (\cf~Section~\ref{sec:simscene_ssf}) a été développée à l'aide de la bibliothèque \emph{D3.js} \cite{d32011}.
...
\label{sec:simscene_eventTexture}

\subsubsection*{Taxonomie}
\label{sec:simscene_getSample}
...
Les classes de sons sont organisées suivant une structure hiérarchique, afin de former une taxonomie. La structure taxonomique s'inspire grandement de l'axe vertical de l'organisation catégorielle proposée par E. Rosch \cite{rosch1978cognition}, \ie~plus le niveau d'abstraction de la classe est élevé, plus la description de la classe est précise, et plus les sources sonores incluses dans cette classe sont semblables. Si le niveau d'abstraction d'une classe est tel que cette dernière possède des sous-classes, alors sa collection de samples est la somme des collections respectives de chacune des sous-classes.

Nous établissons deux taxonomies, une pour les événements (\cf~Figures~\ref{fig:taxonomieEventa} et~\ref{fig:taxonomieEventb}), une autre pour les textures (\cf~Figures~\ref{fig:taxonomieTexturea} et~\ref{fig:taxonomieTextureb}). 

Pour les événements, nous considérons quatre niveaux d'abstraction allant des classes les plus globalisantes (niveau d'abstraction 0), aux classes les plus spécifiques (niveau d'abstraction 3). Pour les textures, nous ne considérons que trois niveaux d'abstraction.
...
Sur la base des taxonomies précédemment établies, 483 sons isolés ont été collectés, dont 381 événements, et 102 textures. Parmi ces samples, 332 ont été enregistrés, et 151 proviennent de différentes banque de sons (\emph{SoundIdeas}\footnote{\emph{SoundIdeas} : \url{ http://www.sound-ideas.com/}} et \emph{Universal SoundBank}\footnote{\emph{Universal SoundBank} : \url{http://www.universal-soundbank.com/}}).
...
L'interface de sélection a fait l'objet d'une étude approfondie dont les résultats sont publiés dans \cite{lafay2016JAES}.

\subsubsection*{Interface de simulation}
...
L'interface de simulation est présentée en détails dans \cite{rossignol2015simscene}.
...
\subsection{Expérience 1.a : planification}
...
\begin{figure}[t]
        \myfloatalign
        \includegraphics[width=1\linewidth]{gfx/ch_4/4}
       \caption{Etapes du processus de simulation}\label{fig:etapeSimu}
\end{figure}
...
Chaque processus de simulation comprend deux parties (\cf~Figure~\ref{fig:etapeSimu}) :
...
\subsection{Expérience 1.b : planification}
...
\subsubsection*{Étude comparative entre les descripteurs sémantiques}
...
\subsubsection*{Étude des espaces de représentation induits par les descripteurs sémantiques}
...
\subsubsection*{L'influence spécifique des marqueurs sonores sur l'agrément perçu}
...
Dans cette expérience, nous notons que les descripteurs structurels ($L$, $L(E)$ et $L(T)$) permettent systématiquement de faire la distinction entre les environnements sonores idéaux et non-idéaux.
...
Parmi les descripteurs structurels spécifiques, calculés en tenant compte des marqueurs sonores, plusieurs permettent maintenant de faire la distinction entre les i-scènes et les ni-scènes: $L_m$, $L_{b}$, $L(E)_{m}$, $L(E)_{b}$, $L(T)_{m}$, $L(T)_{b}$, $L_{m}-L_{b}$ et $L(E)_{m}-L(E)_{b}$.

Parmi ces descripteurs, 7 semblent impacter l'agrément perçu: 
...
\item $L_m$, $L(E)_m$ et $L(T)_b$  ont un impact négatif les ni-scènes.
\end{itemize}
...
Le fait que l'agrément des scènes agréables ne se corrèle pas avec des descripteurs physiques holistiques, contrairement à l'agrément des scènes désagréables, a récemment été déjà observé \cite{gozalo2015relationship}.
...
\subsection{Données et outils statistiques} 
...
\subsubsection{Détection de valeurs extrêmes} 
