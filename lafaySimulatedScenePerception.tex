...
\jlv{Dans la littérature, on fait généralement la distinction entre}
\jls{La littérature fait généralement la distinction entre} :
...
\jls{ \emph{Simscene} tient compte de cette distinction, et \jlv{considère} \jlc{utilise} deux banques de données : l'une regroupant classes d’événements, l'autre des classes de textures. Evénements et textures bénéficient de processus de simulation particuliers.} 

\jls{Les différences morphologiques entre événements et textures sonores ont des conséquences sur la perception des scènes.}  

\jlc{Je reprends ici ma lecture ce matin 20170314. Hier j'ai perdu un peu patience et j'ai, sur les lignes 135 et 137, laissé tomber le système jlv/jlc avant de m'arrêter. Mes corrections: s'agissant de 135 = je retire "différences morphologiques" pour réemployer "distinction", par ailleurs je retire de la deuxième phrase "Les séquences de" pour enchaîner directement sur "Evénements et textures..."; s'agissant de 137 = je réemploie "différences morphologiques". Le discours gagne en fluidité, et au passage, si j'en ai la possibilité, je réduis le texte. C'est dans cet esprit que je continue ma relecture.}

Lorsque des événements émergent d'un environnement sonore, le cerveau traite l'information \jlv{des différentes sources} de manière séparée. Plusieurs flux auditifs sont ainsi générés, \jlv{ \ie~un pour chaque séquence d'événements émis par la même source} \cite{carlyon2004brain}. Inversement, quand le cerveau ne parvient pas à isoler d'événements, les éléments constituants de la scène sont agglomérés dans un même flux. 
\jlc{on vient de poser clairement la problématique: événements/textures. Les notions de 1_ "séquence d'évènements" (j'ai déjà supprimé une locution similaire L135, et j'ajoute par ailleurs que tu abordes dans la phrase juste après le concept de "séquence événementielle selon Maffiolo) 2_ "source" tombent comme un cheveu dans la soupe. C'est trop de vocabulaire d'un coup, et ça brouille un peu les pistes... Tu vois ce que je veux dire ? tu peux reformuler ?}
...
\jlv{Nous nommons sample, un enregistrement d'un son isolé, qu'il s'agisse d'un événement ou d'une texture. Chaque classe est alors comprise comme une collection de samples jugés perceptivement équivalents} \jls{Nous nommons sample, un enregistrement d'un son isolé, événement ou texture. Chaque classe est alors comprise comme une collection de samples jugés perceptivement équivalents}. 

\jlv{Les classes de sons sont organisées suivant une structure hiérarchique, afin de former une taxonomie \jlc{Je relis brièvement "psycho-ea" et je m'interroge. La taxonomie est-elle la finalité de l'organisation hiérarchique des classes (on emploie d'ailleurs plutôt le terme catégories) ou en est-elle une conséquence ? Autrement dit Rosch avait-il pour but de dresser cette taxonomie (comme une nécessité) ou celle-ci est-elle née de sa volonté de hiérarchiser ? Je ne sais pas pourquoi, mais au niveau du sens ça me perturbe}(\cf~Figure~\ref{fig:orgDb}). La structure taxonomique s'inspire \jlc{Exit: grandement} de l'axe vertical de l'organisation catégorielle proposée par E. Rosch \cite{rosch1978cognition}, \ie~plus le niveau d'abstraction de la classe est élevé, plus la description de la classe est précise, et plus les sources sonores incluses dans cette classe sont semblables. Si le niveau d'abstraction d'une classe est tel que cette dernière possède des sous-classes, alors sa collection de samples est la somme des collections respectives de chacune des sous-classes.} 
\jls{Les classes de sons sont organisées suivant une structure hiérarchique(\cf~Figure~\ref{fig:orgDb}) \jlc{est-ce que dans ma correction la référence à la figure (\cf~Figure~\ref{fig:orgDb}) est toujours bien placée/pertinente ?}. Cette structure hiérarchique est largement inspirée de l'axe vertical de l'organisation catégorielle proposée par E. Rosch \cite{rosch1978cognition}, qui dresse une taxonomie des classes ou catégories \ie~plus le niveau d'abstraction de la classe est élevé, plus la description de la classe est précise, et plus les sources sonores incluses dans cette classe sont semblables. Si le niveau d'abstraction d'une classe est tel que cette dernière possède des sous-classes, alors sa collection de samples est la somme des collections respectives de chacune des sous-classes. \jlc{est-ce que dans ma correction ton "\ie~plus le niveau d'abstraction..." est toujours bien placé/pertinent ?} 
...
\jlv{Tous les enregistrements ont été effectués à l’aide d'un micro canon \emph{AT8035}\footnote{\emph{Micro canon AT8035}: \url{eu.audio-technica.com}} relié à un enregistreur \emph{ZOOM H4n}\footnote{\emph{ZOOM H4n} : \url{www.zoom.co.jp/english/products/h4n}}. L’utilisation du micro canon nous permet d’isoler les événements sonores du brouhaha urbain. Pour les textures, il nous permet d’éviter les événements sonores proches du preneur de son. Nous pouvons ainsi pointer des "zones sonores", en nous tenant à une certaine distance de ces dernières, afin de capter uniquement le brouhaha émanant de la zone ciblée}. \jlc{Le fait de préciser le modèle de micro ne nous dispense-t-il pas, dans le cadre de cette communication, d'expliciter encore le pourquoi. Désigner l'AT8035 c'est aussi dire ses qualités. Je propose...} \jls{Exit: L’utilisation du micro canon nous permet d’isoler les événements sonores du brouhaha urbain. Pour les textures, il nous permet d’éviter les événements sonores proches du preneur de son. Nous pouvons ainsi pointer des "zones sonores", en nous tenant à une certaine distance de ces dernières, afin de capter uniquement le brouhaha émanant de la zone ciblée}.
...
L'outil de simulation \jlv{ne propose pas} \jls{n'autorise pas} d’interagir avec un sample en particulier, mais toujours avec une piste, \ie~une séquence de samples. Plusieurs paramètres permettent au sujet de contrôler la structure de chaque piste. 

\begin{itemize}
\item \emph{niveau sonore} ($dB$): \jlv{pour chaque sample, les niveaux sont tirés aléatoirement à partir d'une distribution normale, paramétrée par le sujet en terme de moyenne et de variance} \jls{concerne les samples, les niveaux sont tirés aléatoirement à partir d'une distribution normale, paramétrée par le sujet en terme de moyenne et de variance};
\item \emph{espacement inter-onset} (seconde): \jlv{(piste d'événements seulement) comme pour les niveaux, les espacements sont tirés aléatoirement à partir d'une distribution normale, paramétrée par le sujet en terme de moyenne et de variance} \jls{concerne les pistes d'événements, les espacements sont tirés aléatoirement à partir d'une distribution normale, paramétrée par le sujet en terme de moyenne et de variance};
\item \emph{début et fin} (seconde): \jlv{le sujet fixe le début et la fin de chaque piste} \jls{concerne les pistes d'événements, le sujet fixe le début et la fin de chaque séquence}.
\end{itemize}

Afin de faciliter la simulation, deux paramètres supplémentaires sont proposés:

\jlc{dans ce deuxième itemize on s'y perd un peu entre les "fondu par événement", "piste d'événements" et "sample" non ?}
\begin{itemize}
\item \emph{fondu par événement} (seconde): (piste d'événements seulement) le sujet fixe une durée de fondu (entrée et sortie), appliquée à chaque sample d'une piste d'événements \jlc{"piste d'événements seulement" est contradictoire avec "appliquée à chaque sample d'une piste..." non ?};
...
\jls{Pour coller au premier itemize (version corrigée) je propose l'itemize suivant... en espérant ne pas faire de faute de sens. Tu valideras.} 
\begin{itemize}
\item \emph{fondu par événement} (seconde): concerne les samples, le sujet fixe la durée de fondu (entrée et sortie) de chaque événement};
\item \emph{fondu global} (seconde): concerne les pistes d'événements, le sujet fixe la durée de fondu (entrée et sortie) de la piste. Ce fondu s'applique à l'ensemble des samples de la séquence.
\end{itemize}


Deux de ces paramètres ne s’appliquent que pour les pistes d'événements (\emph{fondu par événement} \jlc{je réitère ma remarque. Si le \emph{fondu par événement} comme son nom l'indique s'applique au niveau de l'événement, il ne s'applique pas au niveau de la piste. Mais peut-être aussi est-ce que je me perds dans tous ces événements/textures/pistes etc. Tu dépatouilles ?} et \emph{espacement inter-onset}), les samples des textures étant séquencés sans espacement.

\subsection{Interface de sélection}
\label{sec:simscene_ssf}

La grande majorité des outils permettant de parcourir une banque de données propose une recherche textuelle sur la base de mots clefs. \jlc{Exit: L’efficacité de ce principe repose avant tout sur la structure typologique, et la nomenclature de la base de données.} Dans le cadre d'une expérience sensorielle visant à objectiver la représentation interne du sujet, cette approche est problématique.
...
\jlc{Concernant ce qui suit et par quoi tu termines ta "section", je m'interroge sur la nécessité de préciser, dans le cadre toujours de cette communication et compte tenu du format texte qui t'est imposé. Ne peut-on élaguer. Ou alors ne peut-on aussi préciser en footnote la question étant: les footnotes sont-elles comptabilisées comme texte ou nous offrent-elles une possibilité tricher ;-) ?} 
Cette organisation visuelle à été pensée afin de:

\begin{enumerate}
\item faciliter le parcours de la banque de données, les sons similaires \jlv{(au sens des classes)} \jls{(au regard des classes)} étant proches les uns des autres.
...
\item le paysage sonore doit être réaliste, au sens de physiquement plausible. \jlv{Autrement dit, le sujet a tout à fait le droit de placer 10 chiens dans son paysage sonore, mais il n’a pas le droit de placer un chien aboyant toutes les 10 millisecondes} \jls{Le sujet peut très bien réunir 10 chiens dans un paysage sonore. Il n'en peut introduire 1 qui aboierait toutes les 10 millisecondes}.
\end{itemize}

\jlc{Exit: Ces contraintes font partie de la consigne. Aucun contrôle n'est fait \emph{a priori} dans l'interface de simulation.}

Chaque processus de simulation comprend deux parties :

\begin{enumerate}
\item la réalisation de la simulation: cette étape \jlv{peut} \jlc{il ne s'agit pas d'une possibilité/éventualité, mais d'une réalité} , elle même, se décomposer en trois actions \jls{cette étape se décompose en trois temps}:
\begin{itemize}
...
\jlc{Exit: Avant de commencer la première simulation, un tutoriel de 20 minutes est proposé aux sujets, afin qu'ils se familiarisent avec le logiciel de simulation, et la banque de données. L'expérience est prévue pour durer 2h30.} 

\subsubsection*{Dispositif expérimental} 
\jlc{la subsubsection Dispositif expérimental me paraît pouvoir être retirée}.
...
\jlc{la subsubsection Participants me paraît pouvoir être retirée}
...
En raison de contraintes temporelles, les sujets n'évaluent que des séquences de 30 secondes des scènes simulées, chacune de ces séquences commençant à la seconde 15, et finissant à le seconde 45 de la scène d'une durée \jlv{d'origine} \jls{initiale} de 1 minute.
...
\jlc{Exit: L'expérience est prévue pour durer 30 minutes. Les sujets ne connaissent pas la nature des scènes.}
...
Chaque scène est \jlv{décrite par un groupe de descripteurs} \jlc{je sais qu'il n'est plus temps de s'arrêter sur les répétitions. Mais bon je ne résiste pas à l'opportunité d'apporter une amélioration. Ne peut-on trouver un autre mot que décrite ?} \jls{A chaque scène est associé un groupe de descripteurs}. C'est sur ces descripteurs que nous pratiquons l'analyse. Un résumé \jlv{des descripteurs} \jls{de ces derniers}, ainsi que des acronymes les désignant, est présenté dans le Tableau~\ref{tab:acronyme}. Afin de rester cohérent avec l’expérience 1.b d'évaluation, les descripteurs ne sont pas calculés sur la durée totale des scènes, mais sur une version réduite de 30 secondes (\cf~Section~\ref{sec:xp1b_plan}). 

Pour chaque scène sonore, trois types de descripteurs sont considérés:

\begin{itemize}
\item \emph{perceptif}: il s'agit de l'agrément perçu de la scène simulée, évalué sur une échelle sémantique de 7 points \jlc{perso, après "évalué" je préciserais ", nous l'avons dit,"...}. Nous notons $\mathcal{A}_{scene}$ l'agrément moyen d'une scène, obtenu en moyennant les notes de tous les sujets. De même, nous notons $\mathcal{A}_{sujet}$ l'agrément par sujet, en moyennant l'ensemble \jlv{de ses notes} \jls{des notes de chacun}. Compte tenu du faible nombre de sujets, nous faisons le choix, dans cette étude, de ne pas normaliser les notes d'agrément;
...
\jlc{dans l'énumération qui suit, les deux premiers items sont exprimés par des verbes, ce qui est cohérent avec l'idée de tests/études. Ne peut-on conserver ce principe jusqu'au bout ?} \jlv{item 3: mesure de l'influence..., item 5: évaluation de espaces..., item 6: mesure de l'influence...}
\jlc{par ailleurs, concernant les items 2 "analyse des descripteurs structurels" et 3 "influence des DS sur l'agrément perçu", si les problématiques sont distinctes, n'y a-t-il redondance dans tes développements ? Pour être précis, est-ce qu'il n'y a pas redite entre "différences significatives entre les descripteurs structurels et/ou l'agrément perçu" et "impact potentiel des descripteurs structurels sur l'agrément perçu" ? Mais c'est peut-être moi qui passe à côté du sens !}
...
\item \emph{influence des descripteurs structurels sur l'agrément perçu}: afin d'évaluer l'impact potentiel des descripteurs structurels sur l'agrément perçu, nous étudions l'existence de corrélations linéaires entre ces deux types de descripteurs \jlv{le "ces deux types de descripteurs" laisse entendre qu'on en a déjà parlé, et qu'on est à même de les identifier. Mais perso je ne sais à quels "deux types" de descripteurs tu fais allusion ?}. Pour mesurer la corrélation, nous utilisons le coefficient de Pearson. Nous adoptons ici une méthodologie couramment utilisée dans l'approche dimensionnelle;
\item \emph{analyse des descripteurs sémantiques}: afin d'apprécier si la distinction affective imposée \jlv{a eu} \jlc{partout ailleurs tu utilises le présent, et ici tu emploies le passé composé ?} un impact sur la composition des scènes en terme de sources sonores, ou, pour être plus précis, s'il existe des classes de sons qui ont été particulièrement utilisées pour simuler un type d'environnement, nous utilisons le V-test. \jlv{Nous vérifions si la présence d'une classe de sons est typique d'un environnement (i ou ni)} \jlc{Les mots changent, mais l'idée est la même. N'y a-t-il pas redite entre cette phrase et celle qui précède ?}. \jls{\emph{analyse des descripteurs sémantiques}: afin d'apprécier si la distinction affective imposée a un impact sur la composition des scènes en terme de sources sonores, s'il est des classes de sons plus particulièrement utilisées pour simuler un type d'environnement (i ou ni), nous utilisons le V-test}. Le test est effectué pour chaque niveau d'abstraction, et séparément, pour les classes d'événements et de textures. Pour chaque classe $j$ et chaque type d'environnement $k$ ($k=\lbrace i,ni\rbrace$), la valeur $V_{jk}$ du V-test se calcule comme suit: 
...
\jlv{Il apparaît que ce sont les événements qui impactent le plus le niveau global des scènes, l'écart entre $L$ et $L(E)$ n'étant que de 1 $dB$ pour les i-scènes et les ni-scènes. Cette observation fait écho aux résultats obtenus par Kuwano~\al~\cite{kuwano_memory_2003}. \jlc{Exit: Au cours de leur expérience, les auteurs demandent à leurs sujets d'évaluer une série d'environnements sonores de manière globale, dans un premier temps, puis d'en évaluer le niveau aux instants où chacun identifie une source sonore. L'étude montre qu'il n'y a pas de différences significatives entre les jugements globaux et les moyennes des jugements instantanés.} Pour en revenir à notre expérience, c'est comme si nos sujets avaient inconsciemment tenu compte de cette réalité perceptive lors de la simulation, en faisant porter le niveau sonore global par des sons courts et bien identifiés, \ie~les événements.}
\jls{Il apparaît que ce sont les événements qui impactent le plus le niveau global des scènes, l'écart entre $L$ et $L(E)$ n'étant que de 1 $dB$ pour les i-scènes et les ni-scènes. C'est comme si nos sujets avaient inconsciemment tenu compte de cette réalité perceptive lors de la simulation, en faisant porter le niveau sonore global par des sons courts et bien identifiés, \ie~les événements. \jlc{dans l'enchainement de ces deux phrases il se peut que voulant élaguer j'ai un peu perdu du sens. Est-ce que le "C'est comme si..." constitue bien un enchaînement logique entre les deux phrases ? Est-ce que "cette réalité perceptive" évoquée dans la seconde est effectivement exprimée dans la seconde ?} Cette observation fait écho aux résultats obtenus par Kuwano~\al~\cite{kuwano_memory_2003}}.
...
\jlv{Cette corrélation élevée, considérant l'ensemble des scènes, résulte du fait que les i-scènes ont tendance à être moins fortes que les ni-scènes, donnant ainsi l'illusion de prolonger la corrélation négative observée pour les ni-scènes.} \jls{Ce résultat, considérant l'ensemble des scènes, s'explique par le fait que les i-scènes ont tendance à être moins fortes que les ni-scènes, donnant ainsi l'illusion de prolonger la corrélation négative observée pour les ni-scènes.}
...
Concernant les textures sonores, 5 marqueurs sont identifiés. Pour les i-scènes, il s'agit de classes faisant référence à des ambiances amorphes, calmes, \jlv{(\emph{cour-intérieur/parc}} \jlc{pourquoi un tiré entre cour et intérieur et un slash entre intérieur et parc ? intérieur est-il pris comme adjectif se rapportant à cour ou comme nom "int"érieur = maison/appartement ?} et \emph{parc}). Pour les ni-scènes, il s'agit, comme pour les événements, de classes faisant référence à des bruits de travaux (\emph{travaux} et \emph{véhicule de travaux}), ainsi que d'une classe faisant référence au trafic (\emph{carrefour}).
...
\begin{figure}[t]
        ...
        \caption{\jlv{Dispersions} \jlc{dispersions au plurial ? Tu confirmes ?} des descripteurs structurels de niveaux sonores relatifs à la présence des marqueurs $L_m$ (a, d), $L(E)_m$ (b, e) et $L(T)_m$ (c, f), en fonction du type de scènes (a, b, c) et de l'agrément perçu $\mathcal{A}_{scene}$ de l'expérience 1.b (d, e, f).}\label{fig:soundlevelMarker}
\end{figure}
...
\begin{itemize}
\item \emph{72 am-scènes}: les 72 scènes précédemment simulées, \jlv{incluant} \jlc{ici j'emploierais plutôt le mot "avec" qui induit l'abréviation "am". Tu n'est pas d'accord ?} les classes de marqueurs (am). Nous notons i/am-scènes, les 36 scènes idéales avec marqueurs, et ni/am-scènes les 36 scènes non-idéales avec marqueurs;
...
\jlc{Exit: L'expérience est prévue pour durer 1 heure. Les sujets ne connaissent pas la nature des scènes.}
