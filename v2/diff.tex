...
%DIF DEL v1/lafaySimulatedScenePerception.tex   Tue Jun 26 15:03:12 2018
%DIF ADD lafaySimulatedScenePerception.tex      Wed Sep  5 14:43:37 2018
\usepackage{url}
...
\begin{frontmatter}
    ...
    \DIFdelbegin %DIFDELCMD < \address{$^{1)}$ Laboratoire des Sciences du Numérique de Nantes-CNRS-\'Ecole Centrale de Nantes, Nantes, France.\\
%DIFDELCMD <     \hspace*{8pt}mathieu.lagrange@cnrs.fr\\
%DIFDELCMD <     $^{2)}$ STMS Ircam-CNRS-UPMC Institut de Recherche et Coordination Acoustique/Musique, Paris, France}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \address{$^{1)}$ Laboratoire des Sciences du Numérique de Nantes-CNRS-\'Ecole Centrale de Nantes, Nantes, France.\\
...
%DIF < Nous pensons que la simulation permet de faire un lien élégant et efficace entre ces deux approches. Il permet en effet d'obtenir du sujet une version modale, \ie~établie sur des dimensions physiques (le signal de la scène simulée), de sa représentation mentale de l'objet; les scènes générées étant à la fois caractérisées sémantiquement (\eg~sources sonores présentes) et physiquement (\eg~niveaux des sources sonores).
%DIF > Nous pensons que la simulation permet de faire un lien élégant et efficace entre ces deux approches. Elle permet en effet d'obtenir du sujet une version modale, \ie~établie sur des dimensions physiques (le signal de la scène simulée), de sa représentation mentale de l'objet; les scènes générées étant à la fois caractérisées sémantiquement (\eg~sources sonores présentes) et physiquement (\eg~niveaux des sources sonores).
...
% \end{enumerate}
 ...
 \begin{enumerate} 
\item \emph{experiment 1.a, simulation}:  the subjects use the soundscape simulator to produce ideal / \DIFdelbegin \DIFdel{non ideal }\DIFdelend \DIFaddbegin \DIFadd{non-ideal }\DIFaddend soundscapes that are considered as material for the following experiments \DIFaddbegin \DIFadd{(the resulting waveforms are available for download}\footnote{\DIFadd{Soundscapes: }\url{https://archive.org/details/soundSimulatedUrbanScene}}\DIFadd{)}\DIFaddend ;
...
To the best of our knowledge, only\DIFdelbegin \DIFdel{Bruce}\DIFdelend ~\cite{bruce2009development,bruce2014effects} considered the use of a simulator to question soundscape perception. They propose a tool that allows the user to modify a given soundscape by adding or removing specific sound sources, by changing the acoustic level of the sources as well as their spatial location. The authors \DIFdelbegin \DIFdel{shows }\DIFdelend \DIFaddbegin \DIFadd{show }\DIFaddend that the addition or removal of the sources globally follows \DIFdelbegin \DIFdel{more }\DIFdelend social or semantic \DIFdelbegin \DIFdel{matters }\DIFdelend \DIFaddbegin \DIFadd{considerations more }\DIFaddend than their acoustical characteristics. A lack of diversity in terms of sound sources is nonetheless mentioned by the authors \DIFdelbegin \DIFdel{that limit }\DIFdelend \DIFaddbegin \DIFadd{as a limiting factor to }\DIFaddend the strength of the outcomes given in this study.
...
In our approach, the simulator developed for this study\footnote{simScene\DIFdelbegin \DIFdel{, available online }\DIFdelend \DIFaddbegin \DIFadd{: }\DIFaddend \url{https://bitbucket.org/mlagrange/simscene}} only yields a monophonic representation of the scene, but that simplification comes with the benefit of a wider range of available sound sources and scheduling parameters in order to provide outputs which are both expressive and useful for analysis.
...
\emph{Simscene} is an online digital audio tool whose first version has been developed as part of the HOULE project\footnote{\DIFdelbegin \emph{\DIFdel{Projet HOULE}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \emph{\DIFadd{HOULE project}}\DIFaddend : \url{houle.ircam.fr}}. It has been designed to run on the popular web browsers \emph{Chrome} and \emph{Firefox}. It is fully written in javascript using the \emph{angular.js} library\footnote{\emph{angular.js}: \url{angularjs.org}} and the \emph{Web Audio} standard that allows the manipulation of digital audio data within the browser\footnote{\emph{Web Audio}: \url{www.w3.org/TR/webaudio}}. The interface for selecting sound sources (\cf~Section~\ref{sec:simscene_ssf}) uses the popular \emph{D3.js} \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{d32011} }%DIFAUXCMD
visualization library }\DIFdelend \DIFaddbegin \DIFadd{visualization library proposed by~\mbox{%DIFAUXCMD
\cite{d32011}}%DIFAUXCMD
}\DIFaddend .
...
The chosen typology is established based on the category/classes of sounds found while reviewing several articles or thesis manuscripts \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{maffiolo_caracterisation_1999,raimbault2002simulation,guastavino_etude_2003,defreville2004aactivity,raimbault2005urban,dubois2006cognitive,devergie_relations_2006,guastavino2006ideal,niessen2010categories,maffiolo_caracterisation_1999,beaumont2004pertinence,polack2008perceptive,leobon_analyse_1986,brown2011towards} }%DIFAUXCMD
}\DIFdelend that study how humans discriminate different kind of urban \DIFdelbegin \DIFdel{soundscapes}\DIFdelend \DIFaddbegin \DIFadd{soundscape, see~\mbox{%DIFAUXCMD
\cite{leobon_analyse_1986,maffiolo_caracterisation_1999,raimbault2002simulation,guastavino_etude_2003,defreville2004aactivity,beaumont2004pertinence,raimbault2005urban,dubois2006cognitive,devergie_relations_2006,guastavino2006ideal,polack2008perceptive,niessen2010categories,brown2011towards} }%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbegin \subsubsection*{\DIFdel{Events and Textures}}
%DIFAUXCMD
\DIFdelend %DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%   DEBUT RELECTURE APPROFONDIE   %%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%DIF < La littérature fait généralement la distinction entre :
%DIF <  \begin{itemize}
%DIF <  \item {un événement sonore}: un son isolé, ponctuel, dont les caractéristiques physiques varient au cours du temps;
%DIF <  \item {une texture sonore}: un son, isolé, long, dont les caractéristiques physiques restent stables au cours du temps \cite{saint1995classification}.
%DIF <  \end{itemize}
\DIFaddbegin \subsubsection*{\DIFadd{Events and Textures}}
\DIFaddend 
...
\item {sound textures}: isolated sounds of long duration whose acoustical characteristics are stable with respect to time\DIFaddbegin \DIFadd{, see~}\DIFaddend \cite{saint1995classification}.
 \end{itemize} 

%DIF <  et utilise deux banques de données distinctes : l'une regroupant des classes d’événements, l'autre des classes de textures. Événements et textures bénéficient de processus de simulation spécifiques.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < Les différences morphologiques entre événements et textures sonores ont des conséquences sur la perception des scènes.
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In order to account for the possibility that the morphological differences between those two classes may have some important consequences on the perception of the scenes, the simulation tool }\emph{\DIFdel{Simscene}} %DIFAUXCMD
\DIFdel{follows this distinction and considers two distinct sound databases: one with classes of events only and the other with classes of textures only. These two types of sound classes also have specific scheduling procedures during the simulation process.
}%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < Lorsque des événements émergent d'un environnement sonore, le cerveau traite l'information des différentes sources de manière séparée \cite{bregman1994auditory}. Plusieurs flux auditifs sont ainsi générés, \ie~un pour chaque séquence d'événements émis par la même source \cite{carlyon2004brain}. Inversement, quand le cerveau ne parvient pas à isoler des événements, les éléments constituants de la scène sont agglomérés dans un même flux.
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{According to the Auditory Scene Analysis (ASA) theory, the brain processes separately events emerging from the different sources of a sound environment \mbox{%DIFAUXCMD
\cite{bregman1994auditory}}%DIFAUXCMD
. Several auditory streams are created, one for each sequence of events emitted by the same source \mbox{%DIFAUXCMD
\cite{carlyon2004brain}}%DIFAUXCMD
, while events that cannot be isolated are grouped into a same stream}\DIFdelend \DIFaddbegin \DIFadd{It indeed appears that the processing of auditory information comprises some sort of decision concerning the nature of the stimuli}\DIFaddend .

%DIF < Maffiolo \cite{maffiolo_caracterisation_1999} met en évidence deux processus de catégorisation distincts, engagés en fonction de la capacité de l'auditeur à identifier des événements sonores. Elle montre l'existence de deux catégories cognitives abstraites d'environnements sonores respectivement appelées: «~les séquences événementielles~» et «~les séquences amorphes~».
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Maffiolo }\DIFdelend \cite{maffiolo_caracterisation_1999} distinguishes two separated categorization processes, either of which is triggered depending on the listener's ability to identify sound events. She shows that those processes lead to two abstract cognitive categories respectively termed "event sequences" and "amorphous sequences". \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < Les séquences événementielles sont des environnements composés d'événements saillants et identifiables (\emph{démarrage de voiture}, \emph{voix d'homme}). Elles profitent d'une analyse descriptive basée sur l'identification des sources sonores. Les séquences amorphes sont des environnements dont il est difficile d'isoler des éléments distincts («~brouhaha de rue~», «~brouhaha de trafic~»). Elles profitent, elles, d'une analyse holistique, effectuée à partir d'indicateurs acoustiques (objectifs) globaux. %Cette différence de traitement entre scènes événementielles et scènes amorphes est relevée également dans \cite{guastavino2006ideal,dubois2006cognitive}.
%DIFDELCMD < 
...
%DIF < Pour ce qui est des textures, \ie~des sons possédants des caractéristiques stables au cours du temps, \cite{mcdermott2011sound,mcdermott2013summary} montrent que le cerveau peut décider sciemment de dégrader l'information physique perçue, en la résumant de manière statistique, faisant fi de toute autre représentation plus détaillée \cite{nelken2013ear}.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Concerning sound textures, \ie~sounds that have stable characteristics over time, \cite{mcdermott2011sound,mcdermott2013summary} demonstrates that the human brain can opt for an abstract, statistical representation of the perceived information, discarding precise physical properties of the sound\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{nelken2013ear}}%DIFAUXCMD
}\DIFdelend .

%DIF < Au regard de ces résultats, il apparaît que le processus de traitement de l'information sonore comprend une prise de décision quant à la nature des stimuli \cite{nelken2013ear,mcdermott2013summary}, laquelle va ensuite influer sur la manière d'extraire et d'analyser l'information.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In the light of those results, it appears that the processing of auditory information comprises some sort of decision concerning the nature of }\DIFdelend \DIFaddbegin \DIFadd{In order to account for the possibility that the morphological differences between sound events and sound textures may have some important consequences on the perception of }\DIFaddend the \DIFdelbegin \DIFdel{stimuli \mbox{%DIFAUXCMD
\cite{nelken2013ear,mcdermott2013summary}}%DIFAUXCMD
.
We thus }\DIFdelend \DIFaddbegin \DIFadd{scenes, the simulation tool }\emph{\DIFadd{Simscene}} \DIFadd{follows this distinction and considers two distinct sound databases: one with classes of events only and the other with classes of textures only. These two types of sound classes also have specific scheduling procedures during the simulation process.
}

\DIFadd{We }\DIFaddend consider in this study the soundscape as a ``\,skeleton of events on a bed of textures\,'' as coined \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{by~}\DIFaddend \cite{nelken2013ear}.

\subsubsection*{Taxonomy}

\begin{figure}[t]
        \myfloatalign
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]{gfx/3.pdf}
%DIFDELCMD <        %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth]{gfx/3.pdf}
       \DIFaddendFL \caption{Hierarchical organisation of the isolated sounds used in the simulation.}\label{fig:orgDb}
\end{figure}
...
The sound classes are organized hierarchically (\cf~Figure~\ref{fig:orgDb}) according to a structure similar to the vertical axis of the categorical organization proposed by\DIFdelbegin \DIFdel{E. Rosch }\DIFdelend \DIFaddbegin \DIFadd{~}\DIFaddend \cite{rosch1978cognition}. The lower the level of abstraction, the more precise the description of the class and the more perceptually similar the sound sources. For classes with a high level of abstraction that have sub classes, their collection of samples is the union of the collections of the sub classes.
...
Accounting for the previously detailed perceptual matters, two taxonomies are built, one for \DIFdelbegin \DIFdel{event sounds }\DIFdelend \DIFaddbegin \DIFadd{sound events }\DIFaddend and one for \DIFdelbegin \DIFdel{texture sounds. Four }\DIFdelend \DIFaddbegin \DIFadd{sound textures. Up to four }\DIFaddend levels of abstraction are considered from the most generic \DIFaddbegin \DIFadd{classes }\DIFaddend (level 0) to the most specific classes (level \DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{3}\DIFaddend ), leading to a taxonomy close to the one used in\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Salamon14}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Salamon14}}%DIFAUXCMD
, see~\ref{app:taxonomie}}\DIFaddend . Only three levels of abstraction are considered for the texture sounds\DIFaddbegin \DIFadd{, see Figure~\ref{fig:taxonomieT}}\DIFaddend .

%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%    FIN RELECTURE APPROFONDIE    %%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin 
...
%DIF < La grande majorité des outils permettant de parcourir une banque de données propose une recherche textuelle sur la base de mots-clé. Dans le cadre d'une expérience sensorielle visant à objectiver la représentation interne du sujet, cette approche est problématique.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < En effet la description verbale du son, si elle est accessible au sujet, peut potentiellement influencer sa sélection. Dans les faits, pour construire une scène environnementale «~calme~», le sujet sélectionne a priori les sons référencés sous le vocable \emph{parc}.
\DIFdelend %DIF > La grande majorité des outils permettant de parcourir une banque de données propose une recherche textuelle sur la base de mots-clé. Dans le cadre d'une expérience sensorielle visant à objectiver la représentation interne du sujet, cette approche est problématique. En effet la description verbale du son, si elle est accessible au sujet, peut potentiellement influencer sa sélection. Dans les faits, pour construire une scène environnementale «~calme~», le sujet sélectionne a priori les sons référencés sous le vocable \emph{parc}.

Once the typology and the set of sounds are available, an important design issue is the need \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend a suitable way to display the sound dataset to the user. Most browsing tools are based on keyword indexing; however, for sensory experiments that study the objectivation of a subject's mental representations, this \DIFdelbegin \DIFdel{means is problematic : indeed, }\DIFdelend \DIFaddbegin \DIFadd{may be problematic as }\DIFaddend the availability of a verbal description of the sound can influence the subject's choice, and potentially induce biases in the analysis conclusions. For example, a subject may automatically select sounds referenced as belonging to a \emph{park} environment to build a \emph{calm} soundscape, rather than focusing on their perception.
...
Therefore, the selection interface considered in this study is \DIFdelbegin \DIFdel{text free }\DIFdelend \DIFaddbegin \DIFadd{text-free }\DIFaddend and designed so as to force the user to rely on listening.
...
%DIF < La scène sonore est ainsi vue comme une somme de sources sonores, ou, si l'on admet la distinction opérée entre événements et textures sonores, «~un squelette d'événements sur un lit de textures~» \cite{nelken2013ear}. Cet outil de simulation est présenté en détail dans \cite{rossignol2015simscene}.
%DIF > La scène sonore est ainsi vue comme une somme de sources sonores, ou, si l'on admet la distinction opérée entre événements et textures sonores, «~un squelette d'événements sur un lit de textures~» \cite{nelken2013ear}. Cet outil de simulation est présenté en détail dans~\cite{rossignol2015simscene}.

As such, the underlying model of the scene is a sum of sound sources. \DIFdelbegin \DIFdel{Further details about the simulation interface can be found in }\DIFdelend \DIFaddbegin \DIFadd{The simulation interface is more thoroughly described by~}\DIFaddend \cite{rossignol2015simscene}.
...
Experiment 1 aims at using a simulation paradigm to investigate the specific influences of the various sound sources constituting urban soundscapes on the perceived pleasantness. For that, the \DIFdelbegin \DIFdel{two first }\DIFdelend \DIFaddbegin \DIFadd{first two }\DIFaddend experiments are planned as follows (\cf~Figure~\ref{fig:xp1_2}):
...
% \end{itemize}
 ...
 \begin{itemize} 
\item \emph{experiment 1.a (simulation)}: during this experiment, subjects are asked to \DIFdelbegin \DIFdel{simulate }\DIFdelend \DIFaddbegin \DIFadd{create simulated }\DIFaddend urban sound environments using \emph{Simscene} (see Section~\ref{sec:simulator}). Each of them has to create two sound environments: one ideal~/ pleasant, and the other \DIFdelbegin \DIFdel{non ideal}\DIFdelend \DIFaddbegin \DIFadd{non-ideal}\DIFaddend ~/ unpleasant.
\item \emph{experiment \DIFdelbegin \DIFdel{2.a }\DIFdelend \DIFaddbegin \DIFadd{1.b }\DIFaddend (evaluation)}: after the simulation phase, only a binary information on the pleasantness property of the respective scenes is available: respectively ideal \DIFdelbegin \DIFdel{(i) or non ideal (ni)}\DIFdelend \DIFaddbegin \DIFadd{or non-ideal}\DIFaddend . Furthermore, this information is given by the creator of the scene. The second experimental step aims at investigating more deeply and more broadly our knowledge on the pleasantness of the simulated scenes. For that, a second group of subjects is asked to evaluate the pleasantness of each scene produced during (1.a), on a semantic scale. This experiment has two goals:
 \begin{enumerate} 
\item to evaluate more precisely the respective influence of the various sources composing the scenes on the pleasantness  (\DIFdelbegin \DIFdel{i or ni}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend ) thanks to a finer quantification of the pleasantness of the scene;
\item to detect the presence of \emph{outliers} or ambiguous scenes. Indeed, throughout our analyses, the predefined hedonic properties of the scenes (\DIFdelbegin \DIFdel{i and ni}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend ) are used as reference. We thus need to ensure beforehand that no ambiguity exists between extreme cases of \DIFdelbegin \DIFdel{i and ni }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal }\DIFaddend scenes, \ie~that the least pleasant \DIFdelbegin \DIFdel{i-scene }\DIFdelend \DIFaddbegin \DIFadd{ideal scene }\DIFaddend remains more highly rated on average than the most pleasant \DIFdelbegin \DIFdel{ni-scene}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scene}\DIFaddend .
 \end{enumerate} 
 \end{itemize} 
...
\DIFaddend % Chaque processus de simulation comprend deux parties :
%
% \begin{enumerate}
...
44 students (30 male, 14 female; averaging $21.6$ years of age, s.d. of $2.0$ years) from \emph{Ecole Centrale de Nantes} (a french engineering school) \DIFdelbegin \DIFdel{took part in }\DIFdelend \DIFaddbegin \DIFadd{performed }\DIFaddend the experiment. All the subjects had been living in Nantes, France, for at least two years at the time of the experiment \DIFaddbegin \DIFadd{and reported normal hearing}\DIFaddend .
...
Among the 44 subjects, 40 succeeded, producing \DIFdelbegin \DIFdel{at }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the end 80 simulated sound scenes (40 ideal scenes, 40 \DIFdelbegin \DIFdel{non ideal }\DIFdelend \DIFaddbegin \DIFadd{non-ideal }\DIFaddend scenes). \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend 4 other subjects were excluded from the process due to a lack of understanding of the instructions or failure to respect them, or for exceeding the maximum duration allowed to perform the experiment.  The software platform used for the experiment, the parametrization of the software platform for each generated scene, as well as a 2 dimensional projection of the resulting scenes are available \DIFdelbegin %DIFDELCMD < \url{http://soundthings.org/research/urbanSoundScape/XP2014}%%%
\DIFdel{. The resulting waveforms are available for download }%DIFDELCMD < \url{https://archive.org/details/soundSimulatedUrbanScene}%%%
\DIFdelend \DIFaddbegin \DIFadd{online}\footnote{\DIFadd{Experiment 1.a: }\url{http://soundthings.org/research/urbanSoundScape/XP2014}}\DIFaddend .
...
\DIFaddend \subsection{Planning of experiment 1.b}
\label{sec:xp1b_plan}
...
\DIFaddbegin \DIFadd{The subjects evaluate the 80 simulated scenes generated in experiment 1.a. }\DIFaddend Due to temporal constraints, subjects only assess 30 seconds of the initial 1-minute simulated scenes \DIFdelbegin \DIFdel{generated in experiment 1.a (from second }\DIFdelend \DIFaddbegin \DIFadd{(from sec. }\DIFaddend 15 to sec. 45).
...
The assessment is done with a 7-point bipolar semantic scale going from -3 (\DIFdelbegin \DIFdel{non ideal }\DIFdelend \DIFaddbegin \DIFadd{non-ideal }\DIFaddend / unpleasant) to +3 (ideal / pleasant). Before evaluating a scene, \DIFaddbegin \DIFadd{the }\DIFaddend subjects must listen \DIFaddbegin \DIFadd{at least }\DIFaddend to the first 20 seconds of the stimuli. After the evaluation, they are free to continue to the next scene.
...
For each participant, sound scenes are played in a quasi random order. 5 ideal scenes and 5 \DIFdelbegin \DIFdel{non ideal }\DIFdelend \DIFaddbegin \DIFadd{non-ideal }\DIFaddend scenes are first sequenced to allow the subjects to calibrate their scores. These first 10 scenes are played back again at the end of the experiment. Only the last evaluations are taken into account. Each participant evaluates all the sound scenes.
...
All the subjects \DIFdelbegin \DIFdel{perform }\DIFdelend \DIFaddbegin \DIFadd{performed }\DIFaddend the experiment on \DIFdelbegin \DIFdel{computers with equivalent }\DIFdelend \DIFaddbegin \DIFadd{standard desktop computers with the same }\DIFaddend hardware and software configurations. The audio files \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend played in diotic conditions by semi-open headphones \emph{Beyerdynamic DT 990 Pro}. The stimuli \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend the scenes obtained in experiment 1.a. The output sound level \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend the same for all the subjects.
...
All the subjects \DIFdelbegin \DIFdel{perform }\DIFdelend \DIFaddbegin \DIFadd{performed }\DIFaddend the experiment simultaneously \DIFdelbegin \DIFdel{. They are }\DIFdelend \DIFaddbegin \DIFadd{in a quiet environment. They were }\DIFaddend not allowed to talk to each other during the experiment.
...
An experimenter \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend available during the whole duration of the experiment in order to assist subjects and \DIFdelbegin \DIFdel{answer questions any questions the subjects may have}\DIFdelend \DIFaddbegin \DIFadd{to answer questions}\DIFaddend .
...
10 students (8 male, 2 female; averaging $23.1$ years of age, s.d. of $1.8$ years) from \emph{Ecole Centrale de Nantes} \DIFdelbegin \DIFdel{took part to }\DIFdelend \DIFaddbegin \DIFadd{performed }\DIFaddend the experiment. All the subjects had been living in Nantes, France, for at least two years at the time of the experiment \DIFaddbegin \DIFadd{and reported normal hearing}\DIFaddend . None of them took part \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the previous simulation experiment (experiment 1.a).
...
\DIFaddend \subsection{Data and statistical analysis}
\label{sec:xp1_dataAna}

\begin{table}[t]
\centering
\begin{tabular}{c c}
Terms                              & \DIFdelbeginFL \DIFdelFL{Acronyms                   }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Abbreviations              }\DIFaddendFL \\
\hline
Sound level                        & $L$                        \\
...
%DIFDELCMD < %%%
\DIFdelFL{ideal/pleasant                     }%DIFDELCMD < & %%%
\DIFdelFL{i                          }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{non-ideal/unpleasant               }%DIFDELCMD < & %%%
\DIFdelFL{ni                         }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{scene ideal/pleasant               }%DIFDELCMD < & %%%
\DIFdelFL{i-scene                    }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{scene non-ideal/unpleasant         }%DIFDELCMD < & %%%
\DIFdelFL{ni-scene                   }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelendFL \hline
\end{tabular}
\vspace{0.5mm}
\caption{\DIFdelbeginFL \DIFdelFL{Acronyms }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Abbreviations }\DIFaddendFL of features used in the analysis of the \DIFdelbeginFL \DIFdelFL{sensory }\DIFdelendFL experiments.}
\label{tab:acronyme}
\end{table}
...
A set of features, upon which the analysis is conducted, is attached to each sound scene. A summary of those features (and the corresponding \DIFdelbegin \DIFdel{acronyms}\DIFdelend \DIFaddbegin \DIFadd{abbreviations}\DIFaddend ) is presented in Table~\ref{tab:acronyme}. In order to be consistent with the evaluation of experiment 1.b, features are not computed on the whole duration of the sequences but only on their 30-second reduced version used as stimuli for experiment 1.b (Section~\ref{sec:xp1b_plan}).
...
% \end{itemize}
 ...
 \begin{itemize} 
\item \emph{perceptual features}: the perceived pleasantness of the composed scene, assessed on a 7-point bipolar semantic scale. $\mathcal{A}_{scene}$ is the average pleasantness for \DIFdelbegin \DIFdel{one scene, }\DIFdelend \DIFaddbegin \DIFadd{each scene, computed }\DIFaddend as the average of \DIFdelbegin \DIFdel{the marks of }\DIFdelend \DIFaddbegin \DIFadd{all the scores given by }\DIFaddend all the subjects \DIFaddbegin \DIFadd{to a specific scene}\DIFaddend . $\mathcal{A}_{subject}$ is the average pleasantness for each \DIFdelbegin \DIFdel{participant}\DIFdelend \DIFaddbegin \DIFadd{subject}\DIFaddend , computed as the average of all the \DIFdelbegin \DIFdel{score given by each participant}\DIFdelend \DIFaddbegin \DIFadd{scores given by a specific subject to all the scenes. $\mathcal{A}_{subject}$ is computed for ideal scenes and non-ideal scenes separately}\DIFaddend . Given the low number of subjects in experiment 1.b, we choose not to normalize the pleasantness scores.
\DIFdelbegin \DIFdel{$\mathcal{A}_{subject}$ is computed for i-scenes and ni-scenes.
}\DIFdelend 

\item \emph{semantic features}: we use a boolean vector $S = (x_1, x_2, \ldots, x_n)$ that indicates the \DIFdelbegin \DIFdel{class }\DIFdelend \DIFaddbegin \DIFadd{classes }\DIFaddend of sounds involved in the scene, \DIFdelbegin \textit{\DIFdel{i.e.}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{\ie~}\DIFaddend the sound classes that are present / absent from the scene. Each boolean $x$ of this vector corresponds to a specific class of sounds: $x = 1$ if the class is present in the scene, and $x = 0$ otherwise. The vector dimension ($n$) depends on the level of abstraction that is considered for the analysis. For instance, for the abstraction level 1, that includes 44 classes of sounds, the dimension is thus $44$ ($n = 44$).

\item \emph{structural features}: while \emph{SimScene} allows us to \DIFdelbegin \DIFdel{acces }\DIFdelend \DIFaddbegin \DIFadd{access }\DIFaddend a variety of information about the scene structure (such as the density of events), we only focus in this first study on the sound levels. To figure those out, we draw inspiration from the $L_{Aeq}$ \DIFaddbegin \DIFadd{measure}\DIFaddend . In our case, it is a scalar computed from the signal (in Volt and not in Pascal), and converted in decibels, with a reference of 1 Volt (full scale). The level is obtained by computing the quadratic mean of the signal every second and averaging the results over the total duration of the scene. An A-filtering module processes the data before the quadratic means are computed. We note $L$, $L(E)$ and $L(T)$ the computed levels by respectively considering the whole set of samples, only the set of event samples, and only the set of texture samples.
 \end{itemize} 
...
\DIFaddend %Pour évaluer l'impact spécifique des différentes sources sonores sur l'agrément perçu, nous soumettons nos travaux aux 5 tests/études de significativité présentés ci-après:
...
\DIFaddend %\item \emph{vérification de l'agrément des scènes simulées}: afin de vérifier que la distinction affective imposée entre les i- et ni-scènes se retrouve au niveau de l'agrément perçu, nous observons s'il existe des différences entre les deux types de scènes au niveau de $\mathcal{A}_{scene}$ et $\mathcal{A}_{sujet}$. La significativité est évaluée par un test de Student à deux populations indépendantes pour $\mathcal{A}_{scene}$, et par un test de Student à deux populations appariées pour $\mathcal{A}_{sujet}$;

\item \emph{Analysis of perceived pleasantness}: the goal is to evaluate whether the perceived pleasantness is in accordance with the pleasantness label given by the creators of the \DIFdelbegin \DIFdel{i- and ni-scenes during the experiment}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes during experiment~}\DIFaddend 1.a. To do so, we consider if there exist significant differences \DIFdelbegin \DIFdel{for the i- and ni-scenes of the average }\DIFdelend \DIFaddbegin \DIFadd{between the ideal and non-ideal scenes for }\DIFaddend $\mathcal{A}_{scene}$ and \DIFdelbegin \DIFdel{the average }\DIFdelend $\mathcal{A}_{subject}$. The significance is evaluated by a two-sample Student test for $\mathcal{A}_{scene}$ and by a paired-sample Student test for $\mathcal{A}_{subject}$.

%DIF < \jfp{je ne comprends pas, pour moi il faut faire, pour chaque sujet de l'expé 1, un test de différence entre Aniscène  et Aiscène a quoi sert Asubject ici ?} \gl{Les deux sont possibles : Ascene mesure des différences structurelles, Asubject des différences perceptives}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  GL comprends pas en francais
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < \item \emph{analyse des descripteurs structurels}: afin d'évaluer si la distinction affective imposée entre les i- et ni-scènes impacte de manière significative la nature des scènes, \ie~s'il existe des différences significatives entre les descripteurs structurels, nous évaluons cette significativité à partir d'un test de Student à deux populations;
\DIFdelend %DIF > \item \emph{analyse des descripteurs structurels}: afin d'évaluer si la distinction affective imposée entre les i- et ni-scènes impacte de manière significative la nature des scènes, \ie~s'il existe des différences significatives entre les descripteurs structurels. Nous évaluons cette significativité à partir d'un test de Student à deux populations;

\item \emph{Analysis of sound levels}: the goal is to evaluate whether the sound levels \DIFaddbegin \DIFadd{($L$, $L(E)$ and $L(T)$) }\DIFaddend differ between the \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . The significance is measured with a two-sample Student test.
...
\item \emph{Influence of sound levels on perceived pleasantness}: the goal is to evaluate \DIFdelbegin \DIFdel{if }\DIFdelend \DIFaddbegin \DIFadd{whether }\DIFaddend the sound levels \DIFdelbegin \DIFdel{impact }\DIFdelend \DIFaddbegin \DIFadd{($L$, $L(E)$ and $L(T)$) affect }\DIFaddend the perceived pleasantness. To do so, we consider linear correlations between \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{those }\DIFaddend features and $\mathcal{A}_{scene}$. The Pearson correlation coefficient is used for that purpose.
...
%DIF < où $c$ est le nombre de classes utilisées, $c_k$ le nombre de classes utilisées pour un type d'environnement $k$, $c_j$ le nombre de classes $j$ utilisées, et $c_{jk}$ le nombre de classes $j$ utilisées pour un type d'environnement $k$. Le V-test teste l'hypothèse nulle que la proportion $\frac{c_{jk}}{c}$ ne diffère pas significativement de la proportion $\frac{c_{jk}}{c_k}$. Si pour un environnement $k$, et une classe $j$, l'hypothèse est rejetée, la classe $j$ est alors typique de l'environnement $k$. Les classes typiques sont nommées \textbf{marqueurs sonores};
%DIF > où $c$ est le nombre de classes utilisées, $c_k$ le nombre de classes utilisées pour un type d'environnement $k$, $c_j$ le nombre de classes $j$ utilisées, et $c_{jk}$ le nombre de classes $j$ utilisées pour un type d'environnement $k$. Le V-test teste l'hypothèse nulle que la proportion $\frac{c_{jk}}{c}$ ne diffère pas significativement de la proportion $\frac{c_{jk}}{c_k}$.

\item \emph{Analysis of semantic features}: the goal is to evaluate \DIFdelbegin \DIFdel{if }\DIFdelend \DIFaddbegin \DIFadd{whether }\DIFaddend specific classes are more frequently used \DIFdelbegin \DIFdel{to simulate }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend a given type of environment (\DIFdelbegin \DIFdel{i or ni}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend ). To do so, a V-test is considered\DIFaddbegin \DIFadd{, see~}\DIFaddend \citep{rakotomalala2008tvpercent}. With $c$ being the total number of classes used \DIFdelbegin \DIFdel{for both types of environment}\DIFdelend \DIFaddbegin \DIFadd{to simulate all the scenes}\DIFaddend , $c_k$ the number of classes used \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{to simulate the scenes of }\DIFaddend a given type of \DIFdelbegin \DIFdel{scene }\DIFdelend \DIFaddbegin \DIFadd{environment }\DIFaddend $k$ (\DIFdelbegin \DIFdel{$k=\lbrace i,ni\rbrace$}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend ), $c_j$ the number of times a class $j$ has been used \DIFaddbegin \DIFadd{to simulate all the scenes}\DIFaddend , and $c_{jk}$ the number of \DIFdelbegin \DIFdel{time }\DIFdelend \DIFaddbegin \DIFadd{times }\DIFaddend a class $j$ has been used \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{to simulate the scenes of }\DIFaddend a given type of \DIFdelbegin \DIFdel{scene }\DIFdelend \DIFaddbegin \DIFadd{environment }\DIFaddend $k$, the V-test evaluates the null hypothesis that the ratio $\frac{c_{jk}}{c}$ is not significantly different from the ratio $\frac{c_{jk}}{c_k}$. For each class $j$, and each environment type $k$, an approximation of the statistical value $V_{jk}$ is computed as follows:

\begin{equation}
V_{jk}=\dfrac{c_{jk}-c_k\frac{c_j}{c}}{\sqrt{c_k\frac{c-c_k}{c-1}\frac{c_j}{c}(1-\frac{c_j}{c})}}
\end{equation}

%DIF >  Si pour un environnement $k$, et une classe $j$, l'hypothèse est rejetée, la classe $j$ est alors typique de l'environnement $k$. Les classes typiques sont nommées \textbf{marqueurs sonores};
\DIFaddbegin 

\DIFaddend If the null hypothesis is rejected, the class $j$ is said to be typical with respect to the type of \DIFdelbegin \DIFdel{scene }\DIFdelend \DIFaddbegin \DIFadd{environnement }\DIFaddend $k$. Such typical classes are called \textbf{sound markers}\DIFaddbegin \DIFadd{, in reference to the work of \mbox{%DIFAUXCMD
\cite{schafer1993soundscape}}%DIFAUXCMD
}\DIFaddend . Testing is done for each class, at each level of abstraction, and separately for texture and event classes.
...
\item \emph{Representation space induced by the semantic features}: the goal is to determine if a representation space of the scenes solely based on the presence / absence of sound sources allows us to distinguish between the two types of \DIFdelbegin \DIFdel{scene}\DIFdelend \DIFaddbegin \DIFadd{scenes}\DIFaddend . Denoting as $S_i$ the semantic features of scene $i$, we compute the distances between all $S_i$ vectors. A Hamming distance is used: considering two $n$-dimension vectors $S_1=(x_{1,1},x_{1,2},\ldots,x_{1,n})$ and $S_2=(x_{2,1},x_{2,2},\ldots,x_{2,n})$, with $x \in\lbrace 0,1\rbrace$, the Hamming distance $d_{ham}$ measures the proportion of coordinates that differ between the two vectors. It is defined as follows:
...
where $\bigoplus$ is the \emph{exclusive-or} operator. Two scenes having similar source compositions will be close in such \DIFaddbegin \DIFadd{a }\DIFaddend space. Using the Hamming distance allows us to take into account equally the presence and absence of classes. In order to measure the intrinsic ability of the space to discriminate between \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend , we use a ranking metric named the precision at rank $k$ ($P@k$). The $P@k$ computes the precision obtained after the $k$ closest items \DIFdelbegin \DIFdel{with respect }\DIFdelend to a given seed item have been found. Formally, for each \DIFdelbegin \DIFdel{$s_i$ scene }\DIFdelend \DIFaddbegin \DIFadd{scene $s_i$ }\DIFaddend (considered as seed), we compute the proportion of $s_j$ scenes in the $k$ nearest neighbors of $s_i$ that share the same label as $s_i$. The $P@k$ is then the average of this ratio for all the items considered as search seeds.
...
First, in order to ensure the coherence of the data, we check that none of the \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend gets a $\mathcal{A}_{scene}$ higher than one of \DIFdelbegin \DIFdel{a i-scene. 4 ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{an ideal scene. Four non-ideal scenes }\DIFaddend do not fulfill that constraint: they are thus removed, together with their corresponding \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend . As a consequence, 36 \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend and 36 \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend remain for analysis. Second, we verify that subjects really perceived a difference in terms of pleasantness between \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . For that, we investigate the mean pleasantness score for each participant $\mathcal{A}_{subject}$, computed separately for each type of environment. It indeed appears that the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend were perceived significantly more pleasant (\DIFdelbegin \DIFdel{$p < 0.01$}\DIFdelend \DIFaddbegin \DIFadd{$p<0.01$}\DIFaddend ) than the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend .

%DIF < \jfp{a la limite on pourrait ne pas écarter arbitrairement de scène et ne garder que celles pour lesquelles les iscenes sont signif. + plaisante que les ni-scene. ca revient au même. c'est les test qui les écarte. il faudrait peut être justifier pourquoi on fait cela  : pour ne pas fausser les résultats de l'analyse et vérifier que les i scènes sont effectivement perçues + plaisantes que les ni.} \gl{Oui. Mais ça revient au même de toute façon}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsubsection*{Analysis of sound levels}
...
\begin{figure}[t]
       ...
       \caption{Distributions of the sound levels $L$ (a, d), $L(E)$ (b, e) \DIFdelbeginFL \DIFdelFL{et }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL $L(T)$ (c, f), with respect to scene type (\DIFaddbeginFL \DIFaddFL{i: ideal, ni: non-ideal) (}\DIFaddendFL a, b, c) and perceived pleasantness $\mathcal{A}_{scene}$ of experiments 1.b (d, e, f).}
\end{figure}
...
First, our analysis focuses on the sound levels. Figures ~\ref{fig:soundlevela},~\ref{fig:soundlevelb} and~\ref{fig:soundlevelc} respectively depict the distributions of levels $L$, $L(E)$ and $L(T)$. There is a significant difference in terms of sound levels between \DIFdelbegin \DIFdel{i- and ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes }\DIFaddend ($L$: $p<0.01$\DIFaddbegin \DIFadd{, mean deviation: -7 $dB$}\DIFaddend ). This difference is significant for events ($L(E)$: $p<0.01$, mean deviation: -7 $dB$) and for textures ($L(T)$: $p<0.01$, mean deviation: -6 $dB$).
...
As expected, the sound level of the sources is indeed a pleasantness indicator, as the \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend tend to be louder. This \DIFdelbegin \DIFdel{results }\DIFdelend \DIFaddbegin \DIFadd{result }\DIFaddend is also an outcome of a large number of related works. We also notice that this difference of sound levels is significant for both events and textures.
...
\DIFaddend %Nous observons, enfin, que le niveau seul ne permet pas de clairement faire la distinction entre les différents types d'environnement. En effet, $20\%$ des i-scènes ont un niveau supérieur au niveau minimal des ni-scènes, alors qu'il n'y a pas de recouvrement, si l'on considère l'agrément perçu $\mathcal{A}_{scene}$.

However, \DIFaddbegin \DIFadd{sound }\DIFaddend level alone is not sufficient to fully differentiate between \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . In fact, $20\%$ of the \DIFdelbegin \DIFdel{i-scenes have a }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes have }\DIFaddend sound levels higher than the lowest level of the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , while there is no overlap when considering the perceived pleasantness $\mathcal{A}_{scene}$.
...
\begin{tabular}{l c c c}
               & all scenes                & \DIFdelbeginFL \DIFdelFL{i-scenes          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ideal scenes          }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{ni-scenes    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{non-ideal scenes      }\DIFaddendFL \\
\hline
...
In this section, more detailed relationships that could exist between sound levels and perceived pleasantness are investigated. Contrary to the previous test, we do not limit ourselves to a binary \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend \emph{vs.} \DIFdelbegin \DIFdel{ni-scenes disctinction}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes distinction}\DIFaddend : we consider here the mean pleasantness $\mathcal{A}_{scene}$ as the perceptual feature. The aim is to investigate the level of correlation between sound levels and $\mathcal{A}_{scene}$. The linear correlation coefficients computed between $\mathcal{A}_{scene}$ \vs~$L$, $L(E)$, $L(T)$ are shown in Table~\ref{tab:corrStructA}. Relationships between $\mathcal{A}_{scene}$ and the sound levels are depicted in \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Figures}\DIFaddend ~\ref{fig:soundleveld},~\ref{fig:soundlevele} and~\ref{fig:soundlevelf}.
...
Concerning $L$, a strong negative correlation with $\mathcal{A}_{scene}$ is measured ($r=-0.77$, $p<0.01$), indicating that the higher the sound level is, the more unpleasant the scene is perceived. \DIFdelbegin \DIFdel{Nevertheless}\DIFdelend \DIFaddbegin \DIFadd{However}\DIFaddend , Figure~\ref{fig:soundleveld} suggests that this relationship does not occur in the same way for \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . In fact, the correlation between $L$ and $\mathcal{A}_{scene}$ \DIFdelbegin \DIFdel{, }\DIFdelend remains high for \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend ($r=-0.78$, $p<0.01$), but is not significant (\DIFaddbegin \DIFadd{$r=-0.32$, }\DIFaddend $p=0.06$) for \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend .
...
When considering the whole set of scenes, the fact that the level is indeed a good indicator of pleasantness can be explained by the fact that the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend tend to be softer than the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , thus allowing us to extend artificially to the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend the negative correlation observed for the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend .
...
\DIFaddend %Nous en concluons que $L$:
%
%\begin{itemize}
...
We thus conclude that $L$:
 ...
 \begin{itemize} 
\item allows us to differentiate between \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend ,
\item characterizes precisely the perceived pleasantness for \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend (unsurprisingly, an unpleasant scene gets all the more unpleasant as it gets louder),
\item is not a relevant feature for modeling the perceived pleasantness of an \textit{a priori} pleasant soundscape (\DIFdelbegin \DIFdel{i-scene}\DIFdelend \DIFaddbegin \DIFadd{ideal scene}\DIFaddend ).
 \end{itemize} 
...
The same conclusions can be drawn for $L(E)$, see~Figure~\ref{fig:soundlevele}. For $L(T)$, as shown on~Figure~\ref{fig:soundlevelf}, the moderate correlation observed for the whole set of scenes \DIFaddbegin \DIFadd{($r=-0.53$, $p<0.01$) }\DIFaddend disappears when separate scenes are considered (\DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend : $r=-0.33$, $p=0.05$, \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend : $r=-0.00$, $p=0.99$). Again, we believe that the negative correlation coming from the whole is an artifact due to the level difference between the two types of scenes (\DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend tend to be softer than \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend ). Thus, while sound event levels maintain a relative ability to predict the pleasantness of the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , texture levels do not bring much information, whatever the type of environment is.
...
\DIFaddend %En résumé, en présence d'un environnement désagréable, les niveaux sonores, en particulier ceux des événements, ont un impact négatif sur l'agrément. En présence d'un environnement agréable, en revanche, aucun des descripteurs structurels considérés ici ne semble influer sur la perception de l'agrément.

To sum up, for an unpleasant environment, sound levels --\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend especially those of events\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend -- negatively influence the perceived pleasantness. On the contrary, for a pleasant environment, none of the sound levels considered in the study seem to influence the perceived pleasantness.

%DIF < Ces premiers résultats pourraient montrer qu'il existe deux modes de perception, mobilisant chacun des descripteurs indépendants, modes qui s'activent en fonction de la nature de l'environnement (i ou ni).
%DIF < 
%DIF < Le fait que $L$ ne permet pas de caractériser l'agrément des i-scènes peut nous amener à penser que toutes les sources sonores ne contribuent pas de manière égale à la perception de l'agrément, mais que seules le niveau de certaines d'entre elles a une réelle influence. Afin d'approfondir ce point, nous analysons, dans la section suivante, les scènes d'un point de vue sémantique, \ie~en nous intéressant à la nature des sources qui les composent.
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%   DEBUT RELECTURE APPROFONDIE   %%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin 

%DIF > Ces premiers résultats pourraient montrer qu'il existe deux modes de perception, mobilisant chacun des descripteurs indépendants, modes qui s'activent en fonction de la nature de l'environnement (i ou ni). Le fait que $L$ ne permet pas de caractériser l'agrément des i-scènes peut nous amener à penser que toutes les sources sonores ne contribuent pas de manière égale à la perception de l'agrément, mais que seules le niveau de certaines d'entre elles a une réelle influence. Afin d'approfondir ce point, nous analysons, dans la section suivante, les scènes d'un point de vue sémantique, \ie~en nous intéressant à la nature des sources qui les composent.
\DIFaddend 

Those first outcomes tend to show that 1) two modes of perception exist depending on the nature of the environment (\DIFdelbegin \DIFdel{i or ni}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend ), and 2) each involves distinct, independent features. The fact that $L$ is not sufficient to characterize the pleasantness of the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend can lead us to conclude that all sound sources do not equally contribute to the perception of pleasantness. We thus put forward the hypothesis that only the level of some of them can influence this perception. In order to investigate further in that direction, we analyze in the next section the scenes from a semantic point of view, \ie\DIFaddbegin \DIFadd{~}\DIFaddend we take an interest in the nature of the sources they are \DIFdelbegin \DIFdel{made }\DIFdelend \DIFaddbegin \DIFadd{composed }\DIFaddend of.
...
\DIFaddend \subsubsection*{Analysis of the semantic features}
...
\begin{figure}[!tp]
  ...
  \subfloat[]{\includegraphics[width=.5\linewidth]{gfx/xp1_class_3}\label{fig:soundsourcec}}
                             \caption{Proportion of simulated scenes (i\DIFdelbeginFL \DIFdelFL{or }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{: ideal, }\DIFaddendFL ni\DIFaddbeginFL \DIFaddFL{: non-ideal}\DIFaddendFL ) involving a specific class of sounds: (a) event classes at an abstraction level $0+$, (b) texture classes at an abstraction level $0$, (c) event sub-classes at an abstraction level $1$ belonging to \emph{traffic} and \emph{public transportation} classes of the abstraction level $0$. }\label{fig:soundsource}
\end{figure}

%DIF < \jfp{pourquoi la classe "start passage way", présente en figure A9, n'est pas présente sur ce graphe ?} \gl{Elle n'est pas parlante}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %Nous analysons la composition des scènes en comptant le nombre de sujets ayant utilisé une classe de sons pour simuler un type d'environnement. Les résultats sont présentés à la figure~\ref{fig:soundsourcea} pour les événements, et à la figure~\ref{fig:soundsourceb} pour les textures. Par souci d'espace, nous choisissons un niveau d'abstraction intermédiaire entre les niveaux 0 et 1, noté $0+$, pour représenter les classes (\cf~Figure~\ref{fig:taxonomie}).

We study the composition of the scenes by counting the number of subjects who used a given class of sounds to simulate a given type of environment (\DIFdelbegin \DIFdel{i or ni}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend ). For the 36 \DIFdelbegin \DIFdel{i and ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal and 36 non-ideal scenes }\DIFaddend considered, results are shown on Figure~\ref{fig:soundsourcea} for events and on Figure~\ref{fig:soundsourceb} for textures. For the sake of clarity, a transitional level of abstraction between level 0 and 1, named $0+$, is used to depict classes, see~Figures~\ref{fig:taxonomieEa}, \ref{fig:taxonomieEb} and \ref{fig:taxonomieT}.
...
We observe a \DIFdelbegin \DIFdel{noticable }\DIFdelend \DIFaddbegin \DIFadd{noticeable }\DIFaddend difference in terms of class choices between the \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . The distribution of the classes is very similar to the one obtained in a related work on ideal urban soundscapes \DIFaddbegin \DIFadd{by~}\DIFaddend \cite{guastavino2006ideal}, \ie~on one hand, classes involving human presence and nature are prevailing in the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend , and on the other hand, classes involving mechanical sounds and/or public works are prevailing in the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend . These results confirm a \DIFdelbegin \DIFdel{previously observed fact}\DIFdelend \DIFaddbegin \DIFadd{fact previously observed by~\mbox{%DIFAUXCMD
\cite{raimbault2005urban} }%DIFAUXCMD
and~\mbox{%DIFAUXCMD
\cite{dubois2006cognitive}}%DIFAUXCMD
}\DIFaddend : the semantic nature of the sound sources play an important role in the assessment of the environment\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{raimbault2005urban,dubois2006cognitive}}%DIFAUXCMD
}\DIFdelend .
...
\DIFaddend %Dans une certaine mesure, nos résultats contredisent ce fait. La figure~\ref{fig:soundsourcea} montre, en effet, que les classes d'événements de \emph{transports publics} (\emph{bus} et \emph{train}, \cf~Figure~\ref{fig:soundsourcec}) ont été utilisées par les sujets, pour des i-scènes, dans $28\%$ des cas, et pour des ni-scènes, dans $42\%$ des cas. Les résultats ne remettent pas en question le fait que les sons de \emph{transports publics} soient bien acceptés: $25\%$ des sujets ont utilisé la classe \emph{bus} pour les i-scènes, un chiffre comparable à celui de la classe \emph{Vélo}, et bien supérieur à celui de toute autre classe de véhicules privés. Cependant les classes \emph{transports publics} sont également bien présentes dans les ni-scènes, plus que les classes \emph{voiture} ou \emph{camion} par exemple. La classe \emph{transports publics} ne peut donc pas être considérée comme typique d'un environnement sonore urbain idéal.

Nevertheless, we notice some differences with \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{guastavino2006ideal}}%DIFAUXCMD
: Guastavino’s results }\DIFdelend \DIFaddbegin \DIFadd{the results of~\mbox{%DIFAUXCMD
\cite{guastavino2006ideal} }%DIFAUXCMD
which }\DIFaddend show that sounds of \emph{public transportation} are specific of ideal urban soundscapes. The authors interpret this by the fact that the perception of pleasantness is, among other things, due to socio-cultural factors. Thus, in our representation of the world, sounds of public transportation \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{would be }\DIFaddend positively connoted and tend to be more accepted than sounds of personal vehicles.

To a certain extent, our results contradict this result. In fact,  Figure~\ref{fig:soundsourcea} shows that \emph{public transportation} classes (\emph{bus} and \emph{train}, \cf~Figure~\ref{fig:soundsourcec}) have been used by the subjects for $28\%$ of the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend , and for $42\%$ of the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend . Those results do not question the fact that sounds of \emph{public transportation} are well accepted: $25\%$ of the subjects used \emph{bus} for the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend , a level that is comparable to the \emph{bike} class, and much higher than all the personal vehicles classes. However, \emph{public transport} classes are also strongly present in the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , for instance more than \emph{light vehicle} or \emph{truck} classes. On the basis of our results, the public transportation class cannot be considered as typical of an ideal urban soundscape.
...
We have shown that, from a qualitative point of view, the composition of the scenes in terms of sound sources differs between \DIFdelbegin \DIFdel{i- or ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal scenes}\DIFaddend . We now investigate whether some of the sound classes are specific to a given environment. For that purpose, the V-test detailed in Section~\ref{sec:xp1_dataAna} is considered separately for each abstraction level. Results are presented in Table~\ref{tab:markers}.
...
Regarding \DIFdelbegin \DIFdel{the }\DIFdelend sound events, 9 markers are identified for all abstraction levels. As shown on Figure~\ref{fig:soundsource}, classes related to human presence (\emph{male footsteps on concrete}, \emph{bicycle bell}), and of nature (\emph{animals, bird}, and \emph{bird song}) are \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend markers as well as the \emph{church bell} class. This latter result may be due to the socio-cultural background of the subjects who are mostly European citizens. In fact, according to Schafer, a sound that is identified by a person as being an important element of his/her environment, is well accepted. Sound markers of \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend are classes related to construction \DIFdelbegin \DIFdel{site }\DIFdelend \DIFaddbegin \DIFadd{sites }\DIFaddend (\emph{construction works}), or suggesting \DIFdelbegin \DIFdel{a strong }\DIFdelend \DIFaddbegin \DIFadd{intense }\DIFaddend traffic (\emph{horn}, \emph{siren}).
...
Regarding \DIFdelbegin \DIFdel{the }\DIFdelend textures, 5 markers are identified. For the \DIFdelbegin \DIFdel{i-scenes, those }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes, those are }\DIFaddend classes related to subdued or quiet ambiances (\emph{courtyard}, \emph{park}). The marker classes for the \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend are, as for the events, related to construction \DIFdelbegin \DIFdel{site }\DIFdelend \DIFaddbegin \DIFadd{sites }\DIFaddend (\emph{construction}, \emph{construction vehicle}), together with a class related to traffic (\emph{crossroads}).
...
\begin{figure}[t]
  ...
  \caption{\DIFaddbeginFL \DIFaddFL{Rank 5-precision (}\DIFaddendFL $P@5$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL obtained by considering the dissimilarity matrix computed from the paired Hamming distances of the semantic features vectors as a function of the abstraction level. The vectors are built by using all the classes ($ET$), only the event classes ($E$), only the texture classes ($T$), only the event classes corresponding to sound markers ($E_m$), or only the event classes excluding sound markers $E_{w/o,m}$. Baseline results are achieved by considering random vectors as input.}\label{fig:pa5}
\end{figure}
...
In this section, we evaluate at which level a semantic representation of the scenes allows us to discriminate between the two types of \DIFdelbegin \DIFdel{environment}\DIFdelend \DIFaddbegin \DIFadd{environments}\DIFaddend . For this purpose, a rank 5-precision \DIFdelbegin \DIFdel{($p@5$) }\DIFdelend is computed on the space induced by the semantic features $S$, and for each abstraction level (see\DIFaddbegin \DIFadd{~}\DIFaddend Section~\ref{sec:xp1_dataAna}). The vectors $S$ are built by using all the classes ($ET$), only the event classes ($E$), only the texture classes ($T$), only the event classes corresponding to sound markers ($E_m$), or only the event classes excluding sound markers ($E_{w/o,m}$). Texture classes corresponding to sound markers are not numerous enough to reliably compute the metric, and are thus not considered. For the same reasons, event classes corresponding to sound markers at abstraction level $0$ are also discarded. Results are shown on Figure~\ref{fig:pa5}.
...
\DIFaddend %En considérant séparément $E$ et $T$, il apparaît 1) que la $p@5$ obtenue avec $E$ est similaire à celle obtenue avec $ET$, 2) que la $p@5$ obtenue avec $T$ est systématiquement inférieure d'environ $10$ à $15\%$ à celle de $E$. Ces résultats indiquent que l'information sémantique permettant de séparer les deux environnements est principalement portée par les événements. Ces résultats font, par ailleurs, écho aux travaux de  \cite{maffiolo_caracterisation_1999}, qui montrent que nous analysons de manière descriptive (en identifiant les sources) les scènes événementielles, \ie~composées d'événements sonores (\cf~Section~\ref{sec:simscene_sampleDataSet}).

Considering $E$ and $T$ separately, it appears that: 1) the \DIFdelbegin \DIFdel{$p@5$ }\DIFdelend \DIFaddbegin \DIFadd{rank 5-precision }\DIFaddend obtained with $E$ is similar to the one obtained with $ET$ ; 2) the \DIFdelbegin \DIFdel{$p@5$ }\DIFdelend \DIFaddbegin \DIFadd{rank 5-precision }\DIFaddend obtained with $T$ is always lower than the one obtained with $E$, by $10\%$ to $15\%$. Those results indicate that the semantic information that is discriminative is mostly carried by the events. Those results are in line with \DIFaddbegin \DIFadd{results of~}\DIFaddend \cite{maffiolo_caracterisation_1999}. As discussed in Section~\ref{sec:simscene_sampleDataSet}, it seems that humans analyze the event scenes which are composed of several sound events in a \DIFdelbegin \DIFdel{causal }\DIFdelend \DIFaddbegin \DIFadd{descriptive }\DIFaddend manner, \ie~by identifying the sources.
...
\DIFaddend %Enfin, il apparaît que la $p@5$ obtenue avec $E_{m}$ est égale, voire supérieure à celles obtenues avec $E$ et $ET$, et ce bien qu'une information partielle soit utilisée dans ce cas pour décrire les scènes. La dimension des vecteurs de description $S$ pour $E_m$ est en effet inférieure à la dimension des vecteurs $S$ pour $E$, qui est elle même inférieure à celle obtenue dans le cas où toutes les classes sont utilisées ($ET$). De plus, dans le cas où les marqueurs ne sont pas pris en compte pour la description ($E_{w/o,m}$), les résultats chutent, passant même en dessous de ceux obtenus en ne considérant que les textures. Cela confirme que la majorité de l'information sémantique permettant de faire la distinction entre i-scènes et ni-scènes est incluse dans les marqueurs.

The dimension of the vectors $S$ for $E_m$ is lower than the dimension of vector $S$ for $E$, itself lower than the dimension of vector $S$ obtained when all the classes are considered ($ET$). $S$ being a boolean vector, the smaller the dimension, the lower the amount of information it can carry. Despite this, it appears that the \DIFdelbegin \DIFdel{$p@5$ }\DIFdelend \DIFaddbegin \DIFadd{rank 5-precision }\DIFaddend obtained with $E_m$ is equal –\DIFdelbegin \DIFdel{or superior }\DIFdelend \DIFaddbegin \DIFadd{\,or superior\,}\DIFaddend – to the ones obtained with $E$ or $ET$, although only a partial information is used in that case to describe the scenes. Reciprocally, if the sound markers are not taken into account for the description ($E_{w/o,m}$), the performance is below the one achieved when considering only the textures as features. Thus, most \DIFaddbegin \DIFadd{if not all }\DIFaddend of the semantic information allowing to differentiate between \DIFdelbegin \DIFdel{i- and ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes }\DIFaddend is included in the markers.
...
To sum up, the outcomes of this analysis are:
 ...
 \begin{enumerate} 
\item unlike what we outlined with the sound levels, a semantic description of the scenes composition in terms of presence~/ absence of sound sources allows us to reliably differentiate between the two types of environments (\DIFdelbegin \DIFdel{i or ni}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend );
\item the semantic information is mainly contained in the event sound classes;
\item only a part of the event classes, \DIFdelbegin \DIFdel{i.e. }\DIFdelend \DIFaddbegin \DIFadd{\ie~}\DIFaddend the sound markers, are useful to differentiate between the \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend .
 \end{enumerate} 
...
Since we have extracted the typical classes of the \DIFdelbegin \DIFdel{i- and ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes }\DIFaddend and verified that the distinction between \DIFdelbegin \DIFdel{them }\DIFdelend \DIFaddbegin \DIFadd{those two types of scenes }\DIFaddend was largely dependent on the presence of these classes, we shall now investigate whether a description of the scenes only based on the sound pressure level of \DIFaddbegin \DIFadd{these }\DIFaddend sound markers could characterize the perceived pleasantness, perhaps better than a globally computed sound level.
...
\DIFaddend \subsubsection*{Influence of sound marker levels on the perceived pleasantness}
...
\begin{figure}[t]
        ...
        \caption{Distribution of the relative sound levels related to the presence of markers $L_m$ (a, d), $L_b$ (b, e) and $L_m-L_b$ (c, f), versus scene type (\DIFaddbeginFL \DIFaddFL{i: ideal, ni: non-ideal) (}\DIFaddendFL a, b, c) and perceived pleasantness $\mathcal{A}_{scene}$ of experiment 1.b (d, e, f).}\label{fig:soundlevelMarker}
\end{figure}
...
To do so, the correlations between $\mathcal{A}_{scene}$ and the sound levels are evaluated. In this section, the sound levels are computed by taking into account only the previously identified sound markers. We define \DIFdelbegin \DIFdel{$X_m$, the $X$ feature }\DIFdelend \DIFaddbegin \DIFadd{$L_m$ (resp. $L(E)_m$ and $L(T)_m$), the sound level }\DIFaddend computed by taking into account only the sound markers, and \DIFdelbegin \DIFdel{$X_b$, the $X$ feature }\DIFdelend \DIFaddbegin \DIFadd{$L_b$ (resp. $L(E)_b$ and $L(T)_b$), the sound level }\DIFaddend computed by taking into account all the sound classes, except the sound markers. When the feature characterizes an \DIFdelbegin \DIFdel{i-scene }\DIFdelend \DIFaddbegin \DIFadd{ideal scene }\DIFaddend (resp. \DIFdelbegin \DIFdel{ni-scene}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scene}\DIFaddend ), only the markers identified for the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend (resp. \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend ) are considered. We henceforth call \emph{\DIFdelbegin \DIFdel{i-markers}\DIFdelend \DIFaddbegin \DIFadd{ideal markers}\DIFaddend } and \emph{\DIFdelbegin \DIFdel{ni-markers}\DIFdelend \DIFaddbegin \DIFadd{non-ideal markers}\DIFaddend } the two types of markers. Results are shown on Table~\ref{tab:corrMarkers}.
...
Concerning the sound levels, the same trends are measured between $L_m$, $L(E)_m$ and $L(T)_m$ on one side, and $L$, $L(E)$ and $L(T)$ on the other side, see~Figures~\ref{fig:soundlevelMarkera} and~\ref{fig:soundlevelMarkerd}\DIFdelbegin \DIFdel{, respectively}\DIFdelend . No matter whether all the classes or only the markers are considered, it appears that:
...
%\end{enumerate}
 ...
 \begin{enumerate} 
\item a significant difference between levels of \DIFdelbegin \DIFdel{i- and ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes }\DIFaddend exists ($L_m$, $L(E)_m$ \DIFdelbegin \DIFdel{et }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $L(T)_m$: $p<0.01$);
\item the sound level of scenes is mainly related to the sound events, compared to the textures;
\item the sound level of events has an influence on the perception of pleasantness for \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , but not for \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend ;
...
\DIFaddend %En conclusion, le niveau des ni-marqueurs a une influence négative sur l'agrément pour les ni-scènes. En revanche le niveau des i-marqueurs n’impacte pas l'agrément perçu pour les i-scènes.

To conclude, the level of \DIFdelbegin \DIFdel{ni- }\DIFdelend \DIFaddbegin \DIFadd{non-ideal }\DIFaddend markers has a negative influence on the pleasantness for the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend . On the other hand, the level of \DIFdelbegin \DIFdel{i- }\DIFdelend \DIFaddbegin \DIFadd{ideal }\DIFaddend markers does not influence the perceived pleasantness for the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend .
...
Considering the non markers classes, we can observe on the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend results, a weak negative correlation for $L_b$  ($r=-52$, $p<0.01$) \DIFdelbegin \DIFdel{et }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $L(E)_b$ ($r=-51$, $p<0.01$), see\DIFaddbegin \DIFadd{~}\DIFaddend Figures~\ref{fig:soundlevelNoisea} and~\ref{fig:soundlevelNoised}. It is the first time that an objective feature allows us to define the pleasantness of \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend . This leads us to conclude that the level of non-typical sound classes of a pleasant environment has a negative influence on the pleasantness.
...
Moreover, whereas $L(T)$ did not show any significant correlation for \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , a strong negative correlation is observed for $L(T)_b$ ($r = -0.73$, $p < 0.01$). This indicates that the level of non-marker texture classes does not influence the perceived pleasantness in the same way for \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . Sound level of textures seems to have a negative effect for the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , and no significant effect for the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend .
...
For the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend , a moderate positive correlation is observed for $L_m-L_b$ ($r=0.67$, $p<0.01$) and $L(E)_m-L(E)_b$ ($r=0.66$, $p<0.01$). \DIFdelbegin \DIFdel{for the ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{For the non-ideal scenes }\DIFaddend no correlation is observed. Thus, for the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend , it is not the absolute markers level that is important, but their relative level with respect to the other sounds composing the scene. A double perceptual mechanism for the ideal environments can thus be observed:
...
%\end{itemize}
 ...
 \begin{itemize} 
\item the higher the absolute level of sounds not being \DIFdelbegin \DIFdel{i-markers}\DIFdelend \DIFaddbegin \DIFadd{ideal markers}\DIFaddend , the weaker the pleasantness,
\item the higher the relative level of \DIFdelbegin \DIFdel{i-markers is, with respect }\DIFdelend \DIFaddbegin \DIFadd{ideal markers compared }\DIFaddend to the remaining sounds, the higher the pleasantness.
 \end{itemize} 
...
On the contrary, the fact that we observe \DIFaddbegin \DIFadd{for the non-ideal scenes both }\DIFaddend significant correlations for $L_m$ and $L(E)_m$ and no correlation for $L_m-L_b$ and $L(E)_m$ - $L(E)_b$\DIFdelbegin \DIFdel{for the ni-scenes}\DIFdelend , shows that \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend is indeed the absolute level that matters \DIFaddbegin \DIFadd{for non-ideal environments}\DIFaddend .
...
\item \emph{differentiating \DIFdelbegin \DIFdel{i- }\DIFdelend \DIFaddbegin \DIFadd{ideal }\DIFaddend and \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend }: the semantic features, and the global sound levels ($L$, $L(E)$ \DIFdelbegin \DIFdel{et }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $L(T)$), allow us to differentiate reliably between \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . The semantic description seems to be more powerful;
...
\item \emph{pleasantness prediction}: considering the correlation between sound levels and pleasantness, it seems that the way subjects perceived the quality of a given environment depends on its very nature (\DIFdelbegin \DIFdel{i or ni}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend ). From the data gathered in those experiments, the same set of features cannot be considered to predict the pleasantness of \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend :
...
\item \emph{for \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend }, the global levels ($L$ \DIFdelbegin \DIFdel{et }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $L(E)$), or the level of sound markers ($L_m$ \DIFdelbegin \DIFdel{et }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $L(E)_{m}$), have a negative influence on pleasantness. Taking into account the contribution of each of the different sources of the scene does not improve the prediction performance compared to a global analysis of the environment.
...
\DIFaddend %Le fait que l'agrément des i-scènes ne se corrèle pas avec des descripteurs physiques holistiques, contrairement à l'agrément des ni-scènes, a récemment été déjà observé \cite{gozalo2015relationship}.

The fact that the pleasantness of the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend is not correlated to global physical features, contrary to the pleasantness of \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , has also been studied recently \DIFaddbegin \DIFadd{by~}\DIFaddend \cite{gozalo2015relationship}.
...
%DIF < \gl{@mathieu, pourquoi pas. Mais je pense qu'on peut laisser également le premier paragraphe, ça permettra aux reviewer de commenter qqch :). Quid de mettre les deux.}
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%    FIN RELECTURE APPROFONDIE    %%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In order to investigate deeper into the relation between the pleasantness of \DIFdelbegin \DIFdel{the ideal and non ideal }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal }\DIFaddend scenes and the sound markers, the audio \DIFdelbegin \DIFdel{waveform }\DIFdelend \DIFaddbegin \DIFadd{waveforms }\DIFaddend of the simulated scenes are regenerated \DIFdelbegin \DIFdel{with or }\DIFdelend without the classes identified as markers. To do so, \DIFdelbegin \DIFdel{i-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal markers }\DIFaddend are removed from the \DIFdelbegin \DIFdel{i-scenes, and ni-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes, and non-ideal markers }\DIFaddend are removed from the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend . To evaluate the impact on the perception of pleasantness caused by those modifications, a perceptual test is conducted with a protocol close to the one considered in experiment 1.b.
...
\DIFaddend %L'objectif est de vérifier si l'absence des marqueurs a un impact sur l'agrément perçu. Deux hypothèses sont formulées:

The objective of this experiment is to study if the removal of the previously identified markers \DIFdelbegin \DIFdel{have }\DIFdelend \DIFaddbegin \DIFadd{has }\DIFaddend an impact on the perceived pleasantness. Two hypothesis are thus formulated:
...
% \end{itemize}
 ...
 \begin{itemize} 
\item \emph{for the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend }, we hypothesize that the absence of \DIFdelbegin \DIFdel{ni-markers }\DIFdelend \DIFaddbegin \DIFadd{non-ideal markers }\DIFaddend will \textbf{increase} the pleasantness score;
\item \emph{for the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend }, we hypothesize that the absence of the \DIFdelbegin \DIFdel{i-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal markers }\DIFaddend will \textbf{decrease} the pleasantness score.
 \end{itemize} 

%DIF < Si la première hypothèse est intuitive, la deuxième l'est moins. En effet, il n’apparaît pas évident que la suppression des i-marqueurs, bien que s'agissant de sons positivement connotés, diminue la qualité globale d'un environnement. Cette suppression aura, de surcroît, pour effet de diminuer le niveau sonore global de la scène.
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%   DEBUT RELECTURE APPROFONDIE   %%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%DIF < Néanmoins, comme nous l'avons vu, le niveau global n'est qu'un indicateur partiel de l'agrément pour les environnements sonores idéaux. Qui plus est, cet indicateur, lorsque qu'il décrit le niveau des i-marqueurs, impacte de manière positive la qualité de la scène. L'hypothèse mérite donc d'être vérifiée.
%DIF > Si la première hypothèse est intuitive, la deuxième l'est moins. En effet, il n’apparaît pas évident que la suppression des i-marqueurs, bien que s'agissant de sons positivement connotés, diminue la qualité globale d'un environnement. Cette suppression aura, de surcroît, pour effet de diminuer le niveau sonore global de la scène. Néanmoins, comme nous l'avons vu, le niveau global n'est qu'un indicateur partiel de l'agrément pour les environnements sonores idéaux. Qui plus est, cet indicateur, lorsque qu'il décrit le niveau des i-marqueurs, impacte de manière positive la qualité de la scène. L'hypothèse mérite donc d'être vérifiée.

If the first hypothesis is rather intuitive, the second is less. Indeed, it is not obvious that the removal of the \DIFdelbegin \DIFdel{i-markers}\DIFdelend \DIFaddbegin \DIFadd{ideal markers}\DIFaddend , though perceptively positively connoted, will decrease the global quality of a soundscape, since this removal also decreases the global sound level of the scene. However, as discussed before, the global amplitude level can only be considered as a partial indicator of pleasantness of the ideal soundscapes. Furthermore, the level of \DIFdelbegin \DIFdel{i-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal markers }\DIFaddend positively impact the pleasantness. For those reasons, the validation of the second hypothesis is of high interest.
...
\DIFaddend %La banque de données de stimuli compte 144 séquences de 30 secondes. Ces 144 séquences comprennent:
%DIF <  \begin{itemize}
%DIF <  \item \emph{72 am-scenes}: les 72 scènes précédemment simulées, avec les classes de marqueurs (am). Nous notons i/am-scènes, les 36 scènes idéales avec marqueurs, et ni/am-scènes les 36 scènes non-idéales avec marqueurs;
%DIF <  \item \emph{72 sm-scènes}: les 72 scènes précédemment simulées, régénérées sans les classes de marqueurs (sm). Nous notons i/sm-scènes, les 36 scènes idéales sans marqueurs, et ni/sm-scènes les 36 scènes non-idéales sans marqueurs.
%DIF <  \end{itemize}
...
\DIFaddend  \begin{itemize} 
\item \emph{72 \DIFdelbegin \DIFdel{km-scenes}\DIFdelend \DIFaddbegin \DIFadd{scenes with markers}\DIFaddend }: the 72 scenes originally simulated by the \DIFdelbegin \DIFdel{subject }\DIFdelend \DIFaddbegin \DIFadd{subjects }\DIFaddend of experiment 1.a where the sound classes identified as markers are kept.
\DIFdelbegin \DIFdel{The 36 ideal scenes with markers are noted i/km-scenes, and the 36 non ideal scenes with markers are noted ni/km-scenes (km for kept markers).
}\DIFdelend \item \emph{72 \DIFdelbegin \DIFdel{rm-scenes}\DIFdelend \DIFaddbegin \DIFadd{scenes without markers}\DIFaddend }:  72 scenes where the sound classes identified as markers are removed.
\DIFdelbegin \DIFdel{The 36 ideal scenes without markers are noted i/rm-scenes, and the 36 non ideal scenes without markers are noted ni/rm-scenes (r for removed markers).
}\DIFdelend  \end{itemize} 

%Nonobstant l'absence des marqueurs, les am- et sm-scènes sont en tout point semblables.

Notwithstanding the presence or absence of markers, the \DIFdelbegin \DIFdel{km-scenes and rm-scenes }\DIFdelend \DIFaddbegin \DIFadd{scenes with and without markers }\DIFaddend are exactly the same.
...
In order to create \DIFdelbegin \DIFdel{rm-scenes }\DIFdelend \DIFaddbegin \DIFadd{the marker-less scenes }\DIFaddend with still some sound diversity and no absence of sound activity \DIFdelbegin \DIFdel{within a long period of }\DIFdelend \DIFaddbegin \DIFadd{for long periods within }\DIFaddend the scene, only the sound classes of events of the first level of abstraction are removed, see Table~\ref{tab:markers}. Those classes are:
...
% \end{itemize}
 ...
 \begin{itemize} 
\item \emph{church bell}, \emph{bicycle bell}, and \emph{animals} for the \DIFdelbegin \DIFdel{i/rm-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes without markers}\DIFaddend ;
\item \emph{siren}, \emph{car horn} for the \DIFdelbegin \DIFdel{ni/rm-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes without markers}\DIFaddend .
 \end{itemize} 

%Il est important de noter ici que tous les i- et ni-marqueurs ne sont donc pas supprimés dans les sm-scènes.

Thus, only part of the \DIFdelbegin \DIFdel{i-markers and ni-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal markers }\DIFaddend are removed from the \DIFdelbegin \DIFdel{rm-scenes}\DIFdelend \DIFaddbegin \DIFadd{scenes without markers}\DIFaddend .

\subsubsection*{\DIFdelbegin \DIFdel{Experiment}\DIFdelend \DIFaddbegin \DIFadd{Procedure}\DIFaddend }
...
%DIF < Pour chaque sujet, les scènes sont présentées dans un ordre aléatoire. Les 10 premières scènes permettent au sujet de calibrer ses notes. Elles sont obligatoirement composées de 5 i/am-scènes et de 5 ni/am-scènes. Ces 10 premières scènes sont rejouées à la fin de l'expérience, et seules les notes données à la deuxième occurrence sont prises en compte.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < L'expérience est prévue pour durer 1 heure. Les sujets ne connaissent pas la nature des scènes.
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The subjects evaluate the 144 scenes. The evaluation is done on a 11-point bipolar semantic scale ranging from -5 (non-ideal / very unpleasant) to +5 (ideal / very pleasant). Before rating a scene, subjects must listen to the first 20 seconds. After scoring, they are free to move on to the next scene.

%DIF > Pour chaque sujet, les scènes sont présentées dans un ordre aléatoire. Les 10 premières scènes permettent au sujet de calibrer ses notes. Elles sont obligatoirement composées de 5 i/am-scènes et de 5 ni/am-scènes. Ces 10 premières scènes sont rejouées à la fin de l'expérience, et seules les notes données à la deuxième occurrence sont prises en compte.
\DIFaddbegin 

\DIFaddend For each subject, the scenes are presented in a random order. The first 10 scenes allow the subject to calibrate their scores. Those calibration scenes are 5 \DIFdelbegin \DIFdel{i/km-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes with markers }\DIFaddend and 5 \DIFdelbegin \DIFdel{ni/km-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes with markers}\DIFaddend . These first 10 scenes are replayed at the end of the experiment, and only the scores given at the second occurrence are taken into account.

%DIF > L'expérience est prévue pour durer 1 heure. Les sujets ne connaissent pas la nature des scènes.
\DIFaddbegin 

\DIFaddend The experiment is scheduled to last 1 hour. The subjects do not know the nature of the scenes.

\subsubsection*{\DIFdelbegin \DIFdel{Experimental protocol}\DIFdelend \DIFaddbegin \DIFadd{Apparatus}\DIFaddend }

%Tous les sujets passent l'expérience sur des machines identiques. L'audio est diffusé en monophonie, par le biais de casques audio semi-ouverts \emph{Beyer-Dynamic DT 990 Pro}. Toutes les scènes sonores ont été re-simulées sur la base des partitions obtenues lors de l'expérience de simulation. Le niveau sonore de sortie est identique pour tous les sujets.

%DIF < Tous les sujets réalisent l'expérience simultanément, dans un environnement calme. Ils n'ont pas le droit de s'adresser la parole pendant l'expérience.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend All the subjects \DIFdelbegin \DIFdel{perform the test at the same time, on the same type of computers, in a quiet environment. They listen to the stimuli with }\DIFdelend \DIFaddbegin \DIFadd{performed the experiment on standard desktop computers with the same hardware and software configurations. The audio files were played in diotic conditions by }\DIFaddend semi-open \DIFdelbegin \emph{\DIFdel{Beyer-Dynamic DT 990 Pro}} %DIFAUXCMD
\DIFdel{headphones , with the sound level set to be }\DIFdelend \DIFaddbegin \DIFadd{headphones }\emph{\DIFadd{Beyerdynamic DT 990 Pro}}\DIFadd{. The output sound level was }\DIFaddend the same for all the subjects.
\DIFdelbegin \DIFdel{They are }\DIFdelend \DIFaddbegin 

%DIF > Tous les sujets réalisent l'expérience simultanément, dans un environnement calme. Ils n'ont pas le droit de s'adresser la parole pendant l'expérience.

\DIFadd{All the subjects performed the experiment simultaneously in a quiet environment. They were }\DIFaddend not allowed to talk to each other during the experiment.
...
\DIFdelbegin \DIFdel{A supervisor is }\DIFdelend \DIFaddbegin \DIFadd{An experimenter was }\DIFaddend available during the whole duration of the experiment \DIFaddbegin \DIFadd{in order to assist subjects and }\DIFaddend to \DIFdelbegin \DIFdel{monitor it and answer to any question the subjects may have}\DIFdelend \DIFaddbegin \DIFadd{answer questions}\DIFaddend .
...
%DIF < 12 sujets (4 femmes) participent à l'expérience. Aucun d'entre eux n'a réalisé l'expérience de simulation, ni la première expérience d'évaluation. Les sujets sont âgés de 22 à 61 ans (moyenne: 29.5, écart-type: 14). Tous les sujets sont Nantais, et vivent dans cette ville depuis deux ans ou plus.Tous les sujets ont réalisé l'expérience avec succès.
%DIF > 12 sujets (4 femmes) participent à l'expérience. Aucun d'entre eux n'a réalisé l'expérience de simulation, ni la première expérience d'évaluation. Les sujets sont âgés de 22 à 61 ans (moyenne: 29.5, écart-type: 14). Tous les sujets sont Nantais, et vivent dans cette ville depuis deux ans ou plus.

12 subjects \DIFdelbegin \DIFdel{perform the test }\DIFdelend \DIFaddbegin \DIFadd{performed the experiment }\DIFaddend (8 male, 4 female; averaging $29.5$ years of age, s.d. of $14$ years). \DIFdelbegin \DIFdel{None of them have performed experiments 1.a and 1.b. }\DIFdelend All the subjects had been living in Nantes, France, for at least two years at the time of the experiment \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{and reported normal hearing. None of them took part in the previous experiments 1.a and 1.b.
}

%DIF > Tous les sujets ont réalisé l'expérience avec succès.

\DIFaddend All the subjects \DIFdelbegin \DIFdel{performed the test successfully}\DIFdelend \DIFaddbegin \DIFadd{succeeded in doing the experiment}\DIFaddend .

\subsection{Data and \DIFaddbegin \DIFadd{statistical }\DIFaddend analysis\DIFdelbegin \DIFdel{method}\DIFdelend }
...
The aim here is to validate the hypothesis that the removal of \DIFdelbegin \DIFdel{i-markers and ni-markers have an impact }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal markers has a significant effect }\DIFaddend on the perceived pleasantness. To do so, we perform \DIFdelbegin \DIFdel{a variance analysis }\DIFdelend \DIFaddbegin \DIFadd{an analysis of variance (ANOVA)}\DIFaddend . We consider $\mathcal{A}_{subject}$ as a dependent variable, and \DIFdelbegin \DIFdel{as independent variables, }\DIFdelend the type of environment (\DIFdelbegin \DIFdel{i/ni) and }\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal) as well as }\DIFaddend the presence/absence of markers \DIFdelbegin \DIFdel{(km/rm)}\DIFdelend \DIFaddbegin \DIFadd{as independent variables}\DIFaddend . As each subject evaluated the whole set of scenes, a two factors repeated \DIFdelbegin \DIFdel{measure ANOVA }\DIFdelend \DIFaddbegin \DIFadd{measures ANOVA with interaction }\DIFaddend is used to evaluate whether there exist a significant difference of perceived pleasantness between \DIFdelbegin \DIFdel{km-scenes and rm-scenes}\DIFdelend \DIFaddbegin \DIFadd{the scenes with and without markers}\DIFaddend . The two independent variables are considered as \emph{within-subject} factors. The factors being of only two levels each (type: \DIFdelbegin \DIFdel{i/ni}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend ,  marker: \DIFdelbegin \DIFdel{km/rm}\DIFdelend \DIFaddbegin \DIFadd{with or without}\DIFaddend ), the sphericity hypothesis does not need to be checked. \emph{Post hoc} analyses are done using the Tukey-Kramer procedure.
%DIF < \jfp{l'analyse est inutile a mon avis, il n'y a que 2 niveaux pour chaque facteur de l'ANOVA.} \gl{2 niveaux par facteur, mais 4 niveaux en tout}

%Tous les tests de significativité sont effectués avec un seuil critique $\alpha=0.05$.

All \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{statistical }\DIFaddend significance tests are \DIFdelbegin \DIFdel{performed with critical threshold }\DIFdelend \DIFaddbegin \DIFadd{conducted with a critical threshold of }\DIFaddend $\alpha=0.05$
\DIFdelbegin \DIFdel{.
...
\begin{figure}[t]
        \myfloatalign
        \subfloat[]
        {\includegraphics[width=.48\linewidth]{gfx/xp4_note_13}\label{fig:xp2_Asubject}}
        \subfloat[]
        {\includegraphics[width=.48\linewidth]{gfx/xp4_note_15}\label{fig:xp2_Ascene}}
        \caption{Distribution of the mean perceived pleasantness per subject $\mathcal{A}_{subject}$ (a) and the mean perceived pleasantness per scene $\mathcal{A}_{scene}$ (b) versus scene type \DIFaddbeginFL \DIFaddFL{(i: ideal, ni: non-ideal, km: with markers, rm: without markers)}\DIFaddendFL . Black stars in subfigure (a) stand for the detected outlier, \ie~subject 7.}\label{fig:xp2A}
\end{figure}

%DIF <  {maj figure: changer am et sm pour km et rm}
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%   DEBUT RELECTURE APPROFONDIE   %%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%DIF < Considérons $\mathcal{A}_{sujet}$ pour les am-scènes. Il apparaît que les réponses d'un des sujets diffèrent des autres. Ce dernier a évalué positivement près de la moitié des ni/am-scènes (\cf~Annexe~\ref{app:xp2} : sujet 7). Le sujet a donné à 58\% des ni/am-scènes une note supérieure à 0, contre une moyenne de 11\% pour les autres sujets. De plus, le sujet a utilisé l'ambitus maximal (-5 à 5) pour noter à la fois les i/ et ni/am-scènes. Ces faits n'ayant pas été observés pour les autres sujets, que l'on considère les expériences 2 ou 1.b, le sujet 7 est éliminé de l'analyse.
%DIF > Considérons $\mathcal{A}_{sujet}$ pour les am-scènes. Il apparaît que les réponses d'un des sujets diffèrent des autres. Ce dernier a évalué positivement plus de la moitié des ni/am-scènes (\cf~Annexe~\ref{app:xp2} : sujet 7). Le sujet a donné à 58\% des ni/am-scènes une note supérieure à 0, contre une moyenne de 11\% pour les autres sujets. De plus, le sujet a utilisé l'ambitus maximal (-5 à 5) pour noter à la fois les i/ et ni/am-scènes. Ces faits n'ayant pas été observés pour les autres sujets, que l'on considère les expériences 2 ou 1.b, le sujet 7 est éliminé de l'analyse.

Let us consider $\mathcal{A}_{subject}$ for the \DIFdelbegin \DIFdel{km-scenes (}\DIFdelend \DIFaddbegin \DIFadd{scenes with markers, see~}\DIFaddend Figure~\ref{fig:xp2_Asubject}\DIFdelbegin \DIFdel{)}\DIFdelend . Close inspection of the answers \DIFdelbegin \DIFdel{show }\DIFdelend \DIFaddbegin \DIFadd{shows }\DIFaddend that one subject's judgments strongly differ from the others'. This subject evaluated positively \DIFdelbegin \DIFdel{close to }\DIFdelend \DIFaddbegin \DIFadd{more than }\DIFaddend half of the \DIFdelbegin \DIFdel{ni/km-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes with markers }\DIFaddend (see Annex~\ref{app:xp2}, subject 7)\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{. He }\DIFaddend gave a score above 0 for 58\% of the \DIFdelbegin \DIFdel{ni/km-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes with markers}\DIFaddend , contrary to an average of 11\% for the other subjects.

Furthermore, this subject used the whole scale (from -5 to 5) to score both the \DIFdelbegin \DIFdel{i-scenes and the ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes and the non-ideal scenes}\DIFaddend . Those behaviors strongly differ with the ones of the other subjects, be they from this experiment or the previous ones. Subject 7 is thus discarded from the analysis.
...
In this section, we study the scores given by the \DIFdelbegin \DIFdel{subject }\DIFdelend \DIFaddbegin \DIFadd{subjects }\DIFaddend while listening to the \DIFdelbegin \DIFdel{several }\DIFdelend \DIFaddbegin \DIFadd{different }\DIFaddend types of scenes, namely the \DIFdelbegin \DIFdel{i/km-scenes, ni/km-scenes, i/rm-scenes and ni/rm-scenes (}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes with markers, non-ideal scenes with markers, ideal scenes without markers and non-ideal scenes without markers, see }\DIFaddend Figure~\ref{fig:xp2_Ascene}\DIFdelbegin \DIFdel{)}\DIFdelend . The repeated \DIFdelbegin \DIFdel{measure }\DIFdelend \DIFaddbegin \DIFadd{measures }\DIFaddend ANOVA applied to $\mathcal{A}_{subject}$ shows a significant effect of the \DIFdelbegin \DIFdel{type environment (i/ni: }\DIFdelend \DIFaddbegin \DIFadd{environment type (}\DIFaddend $F[1,10]=175$, $p<0.01$), of the presence/absence of markers (\DIFdelbegin \DIFdel{km/rm: }\DIFdelend $F[1,10]=7$, $p<0.05$), and of the interaction between those two factors ($F[1,10]=67$, $p<0.01$).

%DIF < The \emph{post hoc} montre, quant à elle, des différences significatives entre tous les groupes d'observations, notamment entre les i/am- et i/sm-scenes ($p<0.05$) et les ni/am- et ni/sm-scenes ($p<0.01$).
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%    FIN RELECTURE APPROFONDIE    %%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%DIF < \jfp{je crois que l'analyse post-hoc n'est pas nécessaire lorsqu'il n'y a que 2 niveaux pour les facteurs de l'ANOVA. les conclusions seront strictement les mêmes que pour l'ANOVA.c'est utile à partir de 3 niveaux, pour savoir quelle moyenne diffère signif. des autres.}
%DIF > The \emph{post hoc} montre, quant à elle, des différences significatives entre tous les groupes d'observations, notamment entre les i/am- et i/sm-scenes ($p<0.05$) et les ni/am- et ni/sm-scenes ($p<0.01$).

The \emph{post hoc} analysis exhibits significant differences between all groups of observations, notably between the \DIFdelbegin \DIFdel{i/km- and the i/rm scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes with markers and the ideal scenes without markers }\DIFaddend ($p<0.05$) as well as between the \DIFdelbegin \DIFdel{ni/km- and the ni/rm-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes with markers and the non-ideal scenes without markers }\DIFaddend ($p<0.01$).
...
\DIFdelbegin \DIFdel{These }\DIFdelend \DIFaddbegin \DIFadd{Those }\DIFaddend results indicate that the removal of the markers indeed modify the perception of the scenes by the subjects\DIFaddbegin \DIFadd{, see Figure~\ref{fig:xp2A}}\DIFaddend . Our two hypotheses are thus verified :
...
% \end{itemize}
 ...
 \begin{itemize} 
\item the removal of \DIFdelbegin \DIFdel{ni-markers }\DIFdelend \DIFaddbegin \DIFadd{non-ideal markers }\DIFaddend improved the pleasantness of the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend ;
\item the removal of \DIFdelbegin \DIFdel{i-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal markers }\DIFaddend reduced the pleasantness of the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend .
 \end{itemize} 

%DIF < L'interaction significative montre que l'effet du type d'environnement influe sur l'effet de l'absence/présence des marqueurs. En effet la moyenne des écarts entre am- et sm-scènes est plus importante pour les ni-scènes ($1.1$) que pour les i-scènes ($0.5$).
%DIF > L'interaction significative montre que l'effet du type d'environnement influe sur l'effet de l'absence/présence des marqueurs. En effet la moyenne des écarts de $\mathcal{A}_{subject}$ entre les am- et sm-scènes est plus importante pour les ni-scènes ($1.1$) que pour les i-scènes ($0.5$).

The significant interaction shows that the type of environment influences the effect of the presence/absence of the markers. Indeed, the average difference \DIFdelbegin \DIFdel{between km-scenes and rm-scenes }\DIFdelend \DIFaddbegin \DIFadd{of $\mathcal{A}_{subject}$ between the scenes with markers and the scenes without markers }\DIFaddend is larger for the \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend ($1.1$) than for the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend ($0.5$).
...
This experiment shows that the presence of the markers identified during the analysis of experiment 1 does have an impact on the perceived pleasantness. The removal of the \DIFdelbegin \DIFdel{ni-markers }\DIFdelend \DIFaddbegin \DIFadd{non-ideal markers }\DIFaddend has a positive effect on the perception of the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend . Perhaps more surprisingly the removal of the \DIFdelbegin \DIFdel{i-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal markers }\DIFaddend slightly decreases the perception of the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend : this is a more striking observation since, due to the removal of the markers, the acoustic pressure level of the \DIFdelbegin \DIFdel{i/km-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes with markers }\DIFaddend is higher than the one of the \DIFdelbegin \DIFdel{i/rm-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes without markers}\DIFaddend .
...
\DIFaddend %Les i-marqueurs ont donc bien un effet bénéfique sur la perception d'un environnement. Le fait que leur suppression diminue $\mathcal{A}_{scene}$ montre clairement qu'il est possible d'améliorer la qualité sonore d'un lieu en ajoutant des sons bien acceptés comme \emph{oiseau}. Ces conclusions vont dans le sens de l'approche positive introduite par Schafer \cite{schafer1977tuning}.

This strongly confirms that \DIFdelbegin \DIFdel{i-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal markers }\DIFaddend do have a positive impact on the perception of an environment. The fact that their removal decreases $\mathcal{A}_{scene}$ \DIFdelbegin \DIFdel{indicate }\DIFdelend \DIFaddbegin \DIFadd{indicates }\DIFaddend that it should be possible to improve the perceived pleasantness of a given urban area by the addition of sounds commonly considered as pleasant such as \emph{bird calls}. Those conclusions are in line with the positive approach introduced by\DIFdelbegin \DIFdel{Schafer in }\DIFdelend \DIFaddbegin \DIFadd{~}\DIFaddend \cite{schafer1977tuning}.
...
\DIFaddend %Les expériences ont montré que la majorité des descripteurs utilisés, qu'ils soient sémantiques ou structurels, permettent de faire la distinction entre une scène idéale et une scène non-idéale.

This series of three experiments showed that most of the descriptors used in this study, be they of a semantic or acoustic nature, allow us to distinguish between an ideal scene and a \DIFdelbegin \DIFdel{non ideal }\DIFdelend \DIFaddbegin \DIFadd{non-ideal }\DIFaddend one.
...
\DIFaddend %Cet état de fait peut potentiellement influer sur les stratégies à adopter pour améliorer la qualité de l’environnement sonore:
...
% \end{itemize}
 ...
 \begin{itemize} 
\item in the case of \DIFdelbegin \DIFdel{non ideal }\DIFdelend \DIFaddbegin \DIFadd{non-ideal }\DIFaddend scenes, one should focus on reducing the acoustic pressure level, whether globally, or by discarding specific sources such as \emph{sirens} or \emph{car horns}
...
\DIFaddend %Ces travaux nous permettent enfin de conjecturer quant à la nature des représentations mentales des concepts «~environnement sonore urbain agréable~» (EA) et «~environnement sonore urbain désagréable~» (ED).

Finally, this work allows us to conjecture as to the nature of the mental representations of the concepts "pleasant \DIFdelbegin \DIFdel{(urban sound ) }\DIFdelend \DIFaddbegin \DIFadd{urban sound }\DIFaddend environment" \DIFdelbegin \DIFdel{(PE) }\DIFdelend and "unpleasant \DIFdelbegin \DIFdel{(urban sound ) }\DIFdelend \DIFaddbegin \DIFadd{urban sound }\DIFaddend environment" \DIFdelbegin \DIFdel{(UE)}\DIFdelend .
...
First, the fact that the semantic information (which sound source is present) and \DIFdelbegin \DIFdel{compositional }\DIFdelend \DIFaddbegin \DIFadd{structural }\DIFaddend information (at which level) are different for ideal and non-ideal scenes leads us to believe that these two types of information characterize the \DIFdelbegin \DIFdel{PE and UE }\DIFdelend \DIFaddbegin \DIFadd{above cited }\DIFaddend concepts.
...
%DIF < Troisièmement, le fait que les descripteurs structurels corrélés à l'agrément diffèrent en fonction du type de scènes nous porte à croire que les caractéristiques structurelles permettant de distinguer deux instances d'un même concept abstrait lié à l'agrément ne sont pas les mêmes pour EA et ED.
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%   DEBUT RELECTURE APPROFONDIE   %%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%DIF < Third, the fact that the compositional descriptors correlated with the perceived pleasantness change according to the type of scenes leads us to believe that the compositional characteristics that allow us to distinguish between two instances of the same abstract concept linked to the pleasantness are not the same for PE and UE.
\DIFaddbegin \section{\DIFadd{Related computational approaches}}

\DIFadd{Considering sound scene synthesis or composition as part of an experimental protocol requires the availability of software resources that are simple to manipulate for subjects with little training, and possibly running on many types of hardware. While most processing frameworks that have been considered are developed in native language for efficiency purposes like Marsyas developed by~\mbox{%DIFAUXCMD
\cite{tzanetakis2000marsyas} }%DIFAUXCMD
or Clam developed by~\mbox{%DIFAUXCMD
\cite{amatriain2006clam}}%DIFAUXCMD
, many advanced sound manipulations can now be performed on modern web browsers using the webaudio library.
}

\DIFadd{Closer to our sound model is the Tapestrea framework proposed by~\mbox{%DIFAUXCMD
\cite{misra2007musical} }%DIFAUXCMD
that allows the manipulation of sounds trough wavelet and sinusoidal modeling for acousmatic composition or sound
...
\DIFadd{The experiments described here does not introduce yet another sound manipulation framework and is thus not designed to be extensible. Rather it is considered here to demonstrate the following:
} \begin{itemize} 
  \item \DIFadd{the use of Webaudio javascript library is useful to design advanced experimental protocols that consider sound manipulation over the web
  }\item \DIFadd{the considered soundscape model is useful to question important research questions within the soundscape community but simple enough to be grasped by novice participants.
} \end{itemize} 

\DIFadd{Extensions to other sound parameters beyond acoustic levels and sound events distribution could be integrated as the software is open source. However, the control of those new parameters would need a specific user experience study to maintain a sufficient degree of usability of the controlling interface.
}
...
\DIFaddend %Dans la continuité des travaux réalisés, il conviendrait de multiplier les expériences de simulation, en faisant varier les qualités affectives (calme, confortable, gênante, \etc), mais aussi en spécifiant des lieux particuliers (parc, place, rue, \etc), afin d'élaborer des corpus entiers de scènes cognitivement renseignées de paysages sonores.
...
\DIFaddend %Il serait par ailleurs intéressant d'utiliser la simulation afin d'étudier plus avant les effets provoqués par la modification volontaire d'une caractéristique d'une scène, comme lors de la suppression des marqueurs sonores pratiquée dans l'expérience 2.
...
%DIF < Outre les possibilités déjà évoquées, le protocole de simulation présenté ici ainsi que son implémentation apporte encore dans ce cas deux avantages:
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%   DEBUT RELECTURE APPROFONDIE   %%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin 

%DIF > Outre les possibilités déjà évoquées, le protocole de simulation présenté ici ainsi que son implémentation apporte encore dans ce cas trois avantages:
\DIFaddend 
...
%DIF <  \item les scènes simulées peuvent être analysées sans avoir à tenir compte des différentes langues maternelles des sujets, la nature sémantique des classes de sons utilisées étant connue \emph{a priori} par l'expérimentateur et sans avoir besoin d'analyser la scène sonore pour idientifier les sources, leurs présences dans la scène étant directement exploitable.
%DIF >  \item les scènes simulées peuvent être analysées sans avoir à tenir compte des différentes langues maternelles des sujets, la nature sémantique des classes de sons utilisées étant connue \emph{a priori} par l'expérimentateur
%DIF >  \item les scènes simulées peuvent être analysées sans avoir besoin d'annoter la scène sonore pour idientifier les sources, leurs présences dans la scène étant directement exploitable.
% \end{itemize}

 \begin{itemize} 
\item The simulator can be deployed on a large scale via the Internet thanks to the web-based software architecture\DIFaddbegin \DIFadd{, provided that the sound databases are adapted to the cultural background of the test subject population}\DIFaddend ;
\item Simulated scenes can be analyzed without the need to take into account the different mother tongues of the subjects, \DIFdelbegin \DIFdel{the semantic nature of the classes of sounds used being }\DIFdelend \DIFaddbegin \DIFadd{since the semantic natures of the used sound classes are }\DIFaddend known \emph{a priori} by the experimenter\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{;
}\item \DIFadd{Simulated scenes can be analyzed }\DIFaddend without the need to \DIFdelbegin \DIFdel{analyze and }\DIFdelend annotate the sound scene to identify the sources, \DIFaddbegin \DIFadd{since }\DIFaddend their occurrences in the simulated scene \DIFdelbegin \DIFdel{being }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend directly available.
 \end{itemize} 

%DIF < Bien évidemment, ces approches nécessitent d'accroître la taille des banques de sons isolés disponibles, un effort conséquent, mais nécessaire, qui contribuera grandement aux nombreux domaines de recherche ayant trait aux scènes sonores.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < \nm{a ce propos, il n'est pas fait mention des sons manquants pour les subjects (cf. protocole expe de 1a);
%DIF < ne faudrait -il pas en dire 2 mots ce qui alimenterait aussi la discussion sur la dependance de la qualite (et l'efficacite) de la simulation a  la constitution de la base de donnees de textures et d'evenemetns ?}
\DIFdelend %DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%    FIN RELECTURE APPROFONDIE    %%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliographystyle{model5-names}
...
\begin{figure}[h]
        ...
        \caption{Distribution of scores given by the subjects during experiment 2 for \DIFdelbeginFL \DIFdelFL{i/am-scenes }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the ideal scenes with markers }\DIFaddendFL (green) \DIFdelbeginFL \DIFdelFL{et ni/am-scenes }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and the non-ideal scenes with markers }\DIFaddendFL (red). The horizontal axis is, from left to right, the 11-point bipolar semantic scale ranging from -5 (non-ideal / very unpleasant) to +5 (ideal / very pleasant).}\label{fig:xp4_note_1}
\end{figure}
