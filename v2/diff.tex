...
%DIF ADD lafaySimulatedScenePerception.tex      Mon Sep  3 11:32:55 2018
\usepackage{url}
...
The assessment is done with a 7-point bipolar semantic scale going from -3 (\DIFdelbegin \DIFdel{non ideal }\DIFdelend \DIFaddbegin \DIFadd{non-ideal }\DIFaddend / unpleasant) to +3 (ideal / pleasant). Before evaluating a scene, subjects must listen to the first 20 seconds of the stimuli. After the evaluation, they are free to continue to the next scene.
...
\item \emph{semantic features}: we use a boolean vector $S = (x_1, x_2, \ldots, x_n)$ that indicates the class of sounds involved in the scene, \DIFdelbegin \textit{\DIFdel{i.e.}} %DIFAUXCMD
...
\item \emph{structural features}: while \emph{SimScene} allows us to \DIFdelbegin \DIFdel{acces }\DIFdelend \DIFaddbegin \DIFadd{access }\DIFaddend a variety of information about the scene structure (such as the density of events), we only focus in this first study on the sound levels. To figure those out, we draw inspiration from the $L_{Aeq}$. In our case, it is a scalar computed from the signal (in Volt and not in Pascal), and converted in decibels, with a reference of 1 Volt (full scale). The level is obtained by computing the quadratic mean of the signal every second and averaging the results over the total duration of the scene. An A-filtering module processes the data before the quadratic means are computed. We note $L$, $L(E)$ and $L(T)$ the computed levels by respectively considering the whole set of samples, only the set of event samples, and only the set of texture samples.
 \end{itemize} 
...
\item \emph{Analysis of perceived pleasantness}: the goal is to evaluate whether the perceived pleasantness is in accordance with the pleasantness label given by the creators of the \DIFdelbegin \DIFdel{i- and ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes }\DIFaddend during the experiment 1.a. To do so, we consider if there \DIFdelbegin \DIFdel{exist significant differences for the i- and ni-scenes of the average }\DIFdelend \DIFaddbegin \DIFadd{exists significant differences between the ideal and non-ideal scenes for }\DIFaddend $\mathcal{A}_{scene}$ and \DIFdelbegin \DIFdel{the average }\DIFdelend $\mathcal{A}_{subject}$. The significance is evaluated by a two-sample Student test for $\mathcal{A}_{scene}$ and by a paired-sample Student test for $\mathcal{A}_{subject}$.
...
\item \emph{Influence of sound levels on perceived pleasantness}: the goal is to evaluate if the sound levels \DIFaddbegin \DIFadd{($L$, $L(E)$ and $L(T)$) }\DIFaddend impact the perceived pleasantness. To do so, we consider linear correlations between \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{those }\DIFaddend features and $\mathcal{A}_{scene}$. The Pearson correlation coefficient is used for that purpose.
...
However, \DIFaddbegin \DIFadd{sound }\DIFaddend level alone is not sufficient to fully differentiate between \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . In fact, $20\%$ of the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend have a sound levels higher than the lowest level of the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , while there is no overlap when considering the perceived pleasantness $\mathcal{A}_{scene}$.
...
Regarding the sound events, 9 markers are identified for all abstraction levels. As shown on Figure~\ref{fig:soundsource}, classes related to human presence (\emph{male footsteps on concrete}, \emph{bicycle bell}), and of nature (\emph{animals, bird}, and \emph{bird song}) are \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend markers as well as the \emph{church bell} class. This latter result may be due to the socio-cultural background of the subjects who are mostly European citizens. In fact, according to Schafer, a sound that is identified by a person as being an important element of his/her environment, is well accepted. Sound markers of \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend are classes related to construction site (\emph{construction works}), or suggesting a strong traffic (\emph{horn}, \emph{siren}).
...
Regarding the textures, 5 markers are identified. For the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend , those classes related to subdued or quiet ambiances (\emph{courtyard}, \emph{park}). The marker classes for the \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend are, as for the events, related to construction site (\emph{construction}, \emph{construction vehicle}), together with a class related to traffic (\emph{crossroads}).
...
Concerning $ET$, the \DIFdelbegin \DIFdel{$p@5$ }\DIFdelend \DIFaddbegin \DIFadd{rank 5-precision }\DIFaddend is $76\%$ at abstraction level $0$ (the most abstract), and remains above $86\%$ for higher abstraction levels. Considering only the presence / absence of sound classes thus allows us to nicely discriminate between the two types of environments. We also notice that the less abstract (and therefore more precise) the description is, the more effective it is to predict agreement.
...
The dimension of the vectors $S$ for $E_m$ is lower than the dimension of vector $S$ for $E$, itself lower than the dimension of vector $S$ obtained when all the classes are considered ($ET$). $S$ being a boolean vector, the smaller the dimension, the lower the amount of information it can carry. Despite this, it appears that the \DIFdelbegin \DIFdel{$p@5$ }\DIFdelend \DIFaddbegin \DIFadd{rank 5-precision }\DIFaddend obtained with $E_m$ is equal – or superior – to the ones obtained with $E$ or $ET$, although only a partial information is used in that case to describe the scenes. Reciprocally, if the sound markers are not taken into account for the description ($E_{w/o,m}$), the performance is below the one achieved when considering only the textures as features. Thus, most of the semantic information allowing to differentiate between \DIFdelbegin \DIFdel{i- and ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes }\DIFaddend is included in the markers.
...
In order to create \DIFdelbegin \DIFdel{rm-scenes }\DIFdelend \DIFaddbegin \DIFadd{the scenes without markers }\DIFaddend with still some sound diversity and no absence of sound activity within a long period of the scene, only the sound classes of events of the first level of abstraction are removed, see Table~\ref{tab:markers}. Those classes are:
...
In this section, we study the scores given by the subject while listening to the \DIFdelbegin \DIFdel{several }\DIFdelend \DIFaddbegin \DIFadd{different }\DIFaddend types of scenes, namely the \DIFdelbegin \DIFdel{i/km-scenes, ni/km-scenes, i/rm-scenes and ni/rm-scenes (}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes with markers, non-ideal scenes with markers, ideal scenes without markers and non-ideal scenes without markers, see }\DIFaddend Figure~\ref{fig:xp2_Ascene}\DIFdelbegin \DIFdel{)}\DIFdelend . The repeated \DIFdelbegin \DIFdel{measure }\DIFdelend \DIFaddbegin \DIFadd{measures }\DIFaddend ANOVA applied to $\mathcal{A}_{subject}$ shows a significant effect of the type environment (\DIFdelbegin \DIFdel{i/ni}\DIFdelend \DIFaddbegin \DIFadd{Type}\DIFaddend : $F[1,10]=175$, $p<0.01$), of the presence/absence of markers (\DIFdelbegin \DIFdel{km/rm}\DIFdelend \DIFaddbegin \DIFadd{marker}\DIFaddend : $F[1,10]=7$, $p<0.05$), and of the interaction between those two factors ($F[1,10]=67$, $p<0.01$).
...
These results indicate that the removal of the markers indeed modify the perception of the scenes by the subjects\DIFaddbegin \DIFadd{, see Figure~\ref{fig:xp2A}}\DIFaddend . Our two hypotheses are thus verified :
...
, many advanced sound manipulations can now be performed on modern web browsers using the webaudio liibrary.
}
...
\DIFadd{The experiments described here does not introduce yet another sound manipulation framework and is thus not designed to be extensible. Rather it is considered here to demonstrate the following:
  ...
  }\item \DIFadd{the soundscape model considered is useful to question important research questions within the soundscape community  but simple enough to be grasped by novice participants.
} \end{itemize} 
...
% \end{itemize}
 ...
 \begin{itemize} 
\item The simulator can be deployed on a large scale via the Internet thanks to the web-based software architecture\DIFaddbegin \DIFadd{, provided that the sound databases are adapted to the cultural background of the cohort}\DIFaddend ;
\item Simulated scenes can be analyzed without the need to take into account the different mother tongues of the subjects, the semantic nature of the \DIFdelbegin \DIFdel{classes of sounds }\DIFdelend \DIFaddbegin \DIFadd{sound classes }\DIFaddend used being known \emph{a priori} by the experimenter\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{;
}\item \DIFadd{Simulated scenes can be analyzed }\DIFaddend without the need to \DIFdelbegin \DIFdel{analyze and }\DIFdelend annotate the sound scene to identify the sources, their occurrences in the simulated scene being directly available.
 \end{itemize} 
