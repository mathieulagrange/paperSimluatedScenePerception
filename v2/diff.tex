...
%DIF ADD lafaySimulatedScenePerception.tex      Wed Jun 27 10:27:03 2018
\usepackage{url}
...
}\DIFdelend \DIFaddbegin \DIFadd{stimuli. We consider in this study the soundscape as a ``\,skeleton of events on a bed of textures\,'' as coined by~\mbox{%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{Maffiolo \mbox{%DIFAUXCMD
\cite{maffiolo_caracterisation_1999} }%DIFAUXCMD
distinguishes two separated categorization processes, either of which is triggered depending on the listener's ability to identify sound events.
She shows that those processes lead to two abstract cognitive categories respectively termed "event sequences" and "amorphous sequences".
}%DIFDELCMD < 
...
\cite{maffiolo_caracterisation_1999} }%DIFAUXCMD
also distinguishes two separated categorization processes, either of which is triggered depending on the listener's ability to identify sound events. She shows that those processes lead to two abstract cognitive categories respectively termed "event sequences" and "amorphous sequences".
}

\DIFadd{In order to account for the possibility that the morphological differences between those two classes may have some important consequences on the perception of the scenes, the simulation tool }\emph{\DIFadd{Simscene}} \DIFadd{follows this distinction and considers two distinct sound databases: one with classes of events only and the other with classes of textures only. These two types of sound classes also have specific scheduling procedures during the simulation process.
}
...
\DIFaddend %Pour ce qui est des textures, \ie~des sons possédants des caractéristiques stables au cours du temps, \cite{mcdermott2011sound,mcdermott2013summary} montrent que le cerveau peut décider sciemment de dégrader l'information physique perçue, en la résumant de manière statistique, faisant fi de toute autre représentation plus détaillée \cite{nelken2013ear}.

\DIFdelbegin \DIFdel{Concerning sound textures, \ie~sounds that have stable characteristics over time, \mbox{%DIFAUXCMD
\cite{mcdermott2011sound,mcdermott2013summary} }%DIFAUXCMD
demonstrates that the human brain can opt for an abstract, statistical representation of the perceived information, discarding precise physical properties of the sound \mbox{%DIFAUXCMD
\cite{nelken2013ear}}%DIFAUXCMD
. }\DIFdelend %DIF > Concerning sound textures, \ie~sounds that have stable characteristics over time,~\cite{mcdermott2011sound,mcdermott2013summary} demonstrates that the human brain can opt for an abstract, statistical representation of the perceived information, discarding precise physical properties of the sound~\cite{nelken2013ear}.
...
%DIFDELCMD < %%%
\DIFdel{In the light of those results, it appears that the processing of auditory information comprises some sort of decision concerning the nature of the stimuli \mbox{%DIFAUXCMD
\cite{nelken2013ear,mcdermott2013summary}}%DIFAUXCMD
. We thus consider in this study the soundscape as a ``\,skeleton of events on a bed of textures \,'' as coined in \mbox{%DIFAUXCMD
\cite{nelken2013ear}}%DIFAUXCMD
. }\DIFdelend 
...
As such, the underlying model of the scene is a sum of sound sources. \DIFdelbegin \DIFdel{Further details about the simulation interface can be found in }\DIFdelend \DIFaddbegin \DIFadd{The simulation interface is more toroughly described by~}\DIFaddend \cite{rossignol2015simscene}.
...
\item \emph{Analysis of perceived pleasantness}: the goal is to evaluate whether the perceived pleasantness is in accordance with the pleasantness label given by the creators of the  i- and \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non ideal scenes }\DIFaddend during the experiment 1.a. To do so, we consider if there exist significant differences for the i- and \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non ideal scenes }\DIFaddend of the average $\mathcal{A}_{scene}$ and the average $\mathcal{A}_{subject}$. The significance is evaluated by a two-sample Student test for $\mathcal{A}_{scene}$ and by a paired-sample Student test for $\mathcal{A}_{subject}$.
...
\item \emph{Analysis of sound levels}: the goal is to evaluate whether the sound levels differ between the i- and \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non ideal scenes}\DIFaddend . The significance is measured with a two-sample Student test.
...
\begin{itemize}
\item allows us to differentiate between i- and \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non ideal scenes}\DIFaddend ,
...
: Guastavino’s results }\DIFdelend \DIFaddbegin \DIFadd{results of~\mbox{%DIFAUXCMD
\cite{guastavino2006ideal} }%DIFAUXCMD
...
We have shown that, from a qualitative point of view, the composition of the scenes in terms of sound sources differs between i- or \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non ideal scenes}\DIFaddend . We now investigate whether some of the sound classes are specific to a given environment. For that purpose, the V-test detailed in  Section~\ref{sec:xp1_dataAna} is considered separately for each abstraction level. Results are presented in Table~\ref{tab:markers}.
...
\item only a part of the event classes, i.e. the sound markers, are useful to differentiate between the \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non ideal scenes}\DIFaddend .
\end{enumerate}
...
To do so, the correlations between $\mathcal{A}_{scene}$ and the sound levels are evaluated. In this section, the sound levels are computed by taking into account only the previously identified sound markers. We define $X_m$, the $X$ feature computed by taking into account only the sound markers, and $X_b$, the $X$ feature computed by taking into account all the sound classes, except the sound markers. When the feature characterizes an \DIFdelbegin \DIFdel{i-scene }\DIFdelend \DIFaddbegin \DIFadd{ideal scene }\DIFaddend (resp. \DIFdelbegin \DIFdel{ni-scene}\DIFdelend \DIFaddbegin \DIFadd{non ideal scene}\DIFaddend ), only the markers identified for the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend (resp. \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non ideal scenes}\DIFaddend ) are considered. We henceforth call \emph{\DIFdelbegin \DIFdel{i-markers}\DIFdelend \DIFaddbegin \DIFadd{ideal markers}\DIFaddend } and \emph{\DIFdelbegin \DIFdel{ni-markers}\DIFdelend \DIFaddbegin \DIFadd{non ideal markers}\DIFaddend } the two types of markers. Results are shown on Table~\ref{tab:corrMarkers}.
...
To conclude, the level of ni- markers has a negative influence on the pleasantness for the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non ideal scenes}\DIFaddend . On the other hand, the level of i- markers does not influence the perceived pleasantness for the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend .
...
\item \emph{differentiating i- and \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non ideal scenes}\DIFaddend }: the semantic features, and the global sound levels ($L$, $L(E)$ et $L(T)$), allow us to differentiate reliably between \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non ideal scenes}\DIFaddend . The semantic description seems to be more powerful;
...
Thus, only part of the \DIFdelbegin \DIFdel{i-markers and ni-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal markers and non ideal markers }\DIFaddend are removed from the rm-scenes.
...
For each subject, the scenes are presented in a random order. The first 10 scenes allow the subject to calibrate their scores. Those calibration scenes are  5 \DIFdelbegin \DIFdel{i/km-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes with markers }\DIFaddend and 5 \DIFdelbegin \DIFdel{ni/km-scenes}\DIFdelend \DIFaddbegin \DIFadd{nideal scenes with markers}\DIFaddend . These first 10 scenes are replayed at the end of the experiment, and only the scores given at the second occurrence are taken into account.
...
Let us consider $\mathcal{A}_{subject}$ for the km-scenes (Figure~\ref{fig:xp2_Asubject}). Close inspection of the answers show that one subject's judgments strongly differ from the others'. This subject evaluated positively close to half of the \DIFdelbegin \DIFdel{ni/km-scenes }\DIFdelend \DIFaddbegin \DIFadd{non ideal scenes with markers }\DIFaddend (see Annex~\ref{app:xp2}, subject 7), and gave a score above 0 for 58\% of the \DIFdelbegin \DIFdel{ni/km-scenes}\DIFdelend \DIFaddbegin \DIFadd{nideal scenes with markers}\DIFaddend , contrary to an average of 11\% for the other subjects.
...
In this section, we study the scores given by the subject while listening to the several types of scenes, namely the \DIFdelbegin \DIFdel{i/km-scenes, ni/km-scenes, i/rm-scenes and ni/rm-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes with markers, nideal scenes with markers, ideal scenes without markers and non ideal scenes without markers }\DIFaddend (Figure~\ref{fig:xp2_Ascene}). The repeated measure ANOVA applied to $\mathcal{A}_{subject}$ shows a significant effect of the type environment (i/ni: $F[1,10]=175$, $p<0.01$), of the presence/absence of markers (km/rm: $F[1,10]=7$, $p<0.05$), and of the interaction between those two factors ($F[1,10]=67$, $p<0.01$).
...
The \emph{post hoc} analysis exhibits significant differences between all groups of observations, notably between the i/km- and the i/rm scenes ($p<0.05$) as well as between the ni/km- and the \DIFdelbegin \DIFdel{ni/rm-scenes }\DIFdelend \DIFaddbegin \DIFadd{non ideal scenes without markers }\DIFaddend ($p<0.01$).
...
The significant interaction shows that the type of environment influences the effect of the presence/absence of the markers. Indeed, the average difference between km-scenes and rm-scenes is larger for the \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non ideal scenes }\DIFaddend ($1.1$) than for the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend ($0.5$).
...
\DIFadd{Extensions to other sound parameters beyond acoustic levels and . However, the control of those new parameters would need a specific user experience study to maintain a high degree of usability of the controlling interface.
}
