...
%DIF DEL ../v1/lafaySimulatedScenePerception.tex   Thu Jun 21 11:33:06 2018
%DIF ADD ../lafaySimulatedScenePerception.tex      Mon Jul 16 11:43:43 2018
\usepackage{url}
...
\DIFdelbegin \DIFdel{One }\DIFdelend \DIFaddbegin \DIFadd{As asserted by~\mbox{%DIFAUXCMD
\cite{aletta2016soundscape}}%DIFAUXCMD
, one }\DIFaddend of the main goals of soundscape studies is to identify which components of the soundscape influence human perception\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{aletta2016soundscape}}%DIFAUXCMD
}\DIFdelend . Establishing a link between an environment (urban sound scene) and the induced human sensation (calmness) is equivalent to instantiating the mental representation of a specific sound scene (a calm urban sound scene). Several means have been considered in the literature to perform this task.
...
The chosen typology is established based on the category/classes of sounds found while reviewing several articles or thesis manuscripts \DIFaddbegin \DIFadd{written by~}\DIFaddend \cite{maffiolo_caracterisation_1999,raimbault2002simulation,guastavino_etude_2003,defreville2004aactivity,raimbault2005urban,dubois2006cognitive,devergie_relations_2006,guastavino2006ideal,niessen2010categories,maffiolo_caracterisation_1999,beaumont2004pertinence,polack2008perceptive,leobon_analyse_1986,brown2011towards} that study how humans discriminate different kind of urban soundscapes.
...
\item {sound textures}: isolated sounds of long duration whose acoustical characteristics are stable with respect to time\DIFaddbegin \DIFadd{, as expressed by~}\DIFaddend \cite{saint1995classification}.
\end{itemize}
...
%DIFDELCMD < %%%
\DIFdel{In order to account for the possibility that the morphological differences between those two classes may have some important consequences on the perception of }\DIFdelend \DIFaddbegin \DIFadd{It indeed appears that the processing of auditory information comprises some sort of decision concerning the nature of }\DIFaddend the \DIFdelbegin \DIFdel{scenes, the simulation tool }\emph{\DIFdel{Simscene}} %DIFAUXCMD
\DIFdel{follows this distinction and considers two distinct sound databases: one with classes of events only and the other with classes of textures only.
These two types of sound classes also have specific scheduling procedures during the simulation process.
}\DIFdelend \DIFaddbegin \DIFadd{stimuli.
}\DIFaddend 
...
, while events that cannot be isolated are grouped into a same stream.
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{In order to account for the possibility that the morphological differences between soud events and sound textures may have some important consequences on the perception of }\DIFaddend the \DIFdelbegin \DIFdel{light of those results, it appears that the processing of auditory information comprises some sort of decision concerning the nature of the stimuli \mbox{%DIFAUXCMD
...
%DIF > Concerning sound textures, \ie~sounds that have stable characteristics over time,~\cite{mcdermott2011sound,mcdermott2013summary} demonstrates that the human brain can opt for an abstract, statistical representation of the perceived information, discarding precise physical properties of the sound~\cite{nelken2013ear}.

%DIF > Au regard de ces résultats, il apparaît que le processus de traitement de l'information sonore comprend une prise de décision quant à la nature des stimuli \cite{nelken2013ear,mcdermott2013summary}, laquelle va ensuite influer sur la manière d'extraire et d'analyser l'information.

%DIF >  mcdermott2013summary

\DIFaddend \subsubsection*{Taxonomy}
...
\cite{Salamon14}}%DIFAUXCMD
, see Figure~\ref{fig:taxonomieEb}}\DIFaddend . Only three levels of abstraction are considered for the texture sounds\DIFaddbegin \DIFadd{, see Figure~\ref{fig:taxonomieT}}\DIFaddend .
...
\DIFdelbegin \DIFdel{The resulting waveforms are available for download }%DIFDELCMD < \url{https://archive.org/details/soundSimulatedUrbanScene}%%%
\DIFdel{.
}\DIFdelend 
...
All the subjects perform the experiment simultaneously. They \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend not allowed to talk to each other during the experiment.
...
%DIFDELCMD < %%%
\DIFdelendFL %DIF >                                     &                            \\
%DIF >  ideal/pleasant                     & i                          \\
%DIF >  non-ideal/unpleasant               & ni                         \\
%DIF >  scene ideal/pleasant               & i-scene                    \\
%DIF >  scene non-ideal/unpleasant         & ni-scene                   \\
\hline
\end{tabular}
\vspace{0.5mm}
...
\item \emph{Analysis of perceived pleasantness}: the goal is to evaluate whether the perceived pleasantness is in accordance with the pleasantness label given by the creators of the  \DIFdelbegin \DIFdel{i- and ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes }\DIFaddend during the experiment 1.a. To do so, we consider if there \DIFdelbegin \DIFdel{exist significant differences for the i- and ni-scenes of the }\DIFdelend \DIFaddbegin \DIFadd{exists significant differences between the }\DIFaddend average $\mathcal{A}_{scene}$ and the average $\mathcal{A}_{subject}$ \DIFaddbegin \DIFadd{of the ideal and non-ideal scenes}\DIFaddend . The significance is evaluated by a two-sample Student test for $\mathcal{A}_{scene}$ and by a paired-sample Student test for $\mathcal{A}_{subject}$.
...
\item \emph{Analysis of sound levels}: the goal is to evaluate whether the sound levels differ between the \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . The significance is measured with a two-sample Student test.
...
\item \emph{Analysis of semantic features}: the goal is to evaluate if specific classes are more frequently used to simulate a given type of environment (\DIFdelbegin \DIFdel{i or ni}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend ). To do so, a V-test is considered\DIFaddbegin \DIFadd{, see~}\DIFaddend \citep{rakotomalala2008tvpercent}. With $c$ being the total number of classes used for both types of environment, $c_k$ the number of classes used for a given type of \DIFdelbegin \DIFdel{scene }\DIFdelend \DIFaddbegin \DIFadd{environment }\DIFaddend $k$ (\DIFdelbegin \DIFdel{$k=\lbrace i,ni\rbrace$}\DIFdelend \DIFaddbegin \DIFadd{$k \in \lbrace ideal, non-ideal\rbrace$}\DIFaddend ), $c_j$ the number of times a class $j$ has been used, and $c_{jk}$ the number of time a class $j$ has been used for a given type of \DIFdelbegin \DIFdel{scene }\DIFdelend \DIFaddbegin \DIFadd{environment }\DIFaddend $k$, the V-test evaluates the null hypothesis that the ratio $\frac{c_{jk}}{c}$ is not significantly different from the ratio $\frac{c_{jk}}{c_k}$. For each class $j$, and each environment type $k$, an approximation of the statistical value $V_{jk}$ is computed as follows:
...
\begin{figure}[t]
       ...
       \caption{Distributions of the sound levels $L$ (a, d), $L(E)$ (b, e) et $L(T)$ (c, f), with respect to scene type (\DIFaddbeginFL \DIFaddFL{i: ideal, ni: non-ideal) (}\DIFaddendFL a, b, c) and perceived pleasantness $\mathcal{A}_{scene}$ of experiments 1.b (d, e, f).}
\end{figure}
...
First, our analysis focuses on the sound levels. Figures ~\ref{fig:soundlevela},~\ref{fig:soundlevelb} and~\ref{fig:soundlevelc} respectively depict the distributions of levels $L$, $L(E)$ and $L(T)$. There is a significant difference in terms of sound levels between \DIFdelbegin \DIFdel{i- and ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes }\DIFaddend ($L$: $p<0.01$). This difference is significant for events ($L(E)$: $p<0.01$, mean deviation: -7 $dB$) and for textures ($L(T)$: $p<0.01$, mean deviation: -6 $dB$).
...
However, level alone is not sufficient to fully differentiate between \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . In fact, $20\%$ of the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend have a sound levels higher than the lowest level of the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , while there is no overlap when considering the perceived pleasantness $\mathcal{A}_{scene}$.
...
Concerning $L$, a strong negative correlation with $\mathcal{A}_{scene}$ is measured ($r=-0.77$, $p<0.01$), indicating that the higher the sound level is, the more unpleasant the scene is perceived. Nevertheless, Figure~\ref{fig:soundleveld} suggests that this relationship does not occur in the same way for \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . In fact, the correlation between $L$ and $\mathcal{A}_{scene}$, remains high for \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend ($r=-0.78$, $p<0.01$), but is not significant ($p=0.06$) for \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend .
...
The same conclusions can be drawn for $L(E)$, see~Figure~\ref{fig:soundlevele}. For $L(T)$, as shown on~Figure~\ref{fig:soundlevelf}, the moderate correlation observed for the whole set of scenes disappears when separate scenes are considered (\DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend : $r=-0.33$, $p=0.05$, \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend : $r=-0.00$, $p=0.99$). Again, we believe that the negative correlation coming from the whole is an artifact due to the level difference between the two types of scenes (\DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend tend to be softer than \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend ). Thus, while sound event levels maintain a relative ability to predict the pleasantness of the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , texture levels do not bring much information, whatever the type of environment is.
...
Since we have extracted the typical classes of the \DIFdelbegin \DIFdel{i- and ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes }\DIFaddend and verified that the distinction between them was largely dependent on the presence of these classes, we shall now investigate whether a description of the scenes only based on the sound pressure level of sound markers could characterize the perceived pleasantness, perhaps better than a globally computed sound level.
...
\begin{enumerate}
\item a significant difference between levels of \DIFdelbegin \DIFdel{i- and ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes }\DIFaddend exists ($L_m$, $L(E)_m$ et $L(T)_m$: $p<0.01$);
...
To conclude, the level of ni- markers has a negative influence on the pleasantness for the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend . On the other hand, the level of \DIFdelbegin \DIFdel{i- }\DIFdelend \DIFaddbegin \DIFadd{the ideal }\DIFaddend markers does not influence the perceived pleasantness for the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend .
...
Considering the non markers classes, we can observe on the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend results, a weak negative correlation for $L_b$  ($r=-52$, $p<0.01$) et $L(E)_b$ ($r=-51$, $p<0.01$), see Figures~\ref{fig:soundlevelNoisea}  and ~\ref{fig:soundlevelNoised}. It is the first time that an objective feature allows us to define the pleasantness of \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend . This leads us to conclude that the level of non-typical sound classes of a pleasant environment has a negative influence on the pleasantness.
...
For the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend , a moderate positive correlation is observed for $L_m-L_b$ ($r=0.67$, $p<0.01$) and $L(E)_m-L(E)_b$ ($r=0.66$, $p<0.01$). for the \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend no correlation is observed. Thus, for the \DIFdelbegin \DIFdel{i-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal scenes}\DIFaddend , it is not the absolute markers level that is important, but their relative level with respect to the other sounds composing the scene. A double perceptual mechanism for the ideal environments can thus be observed:
...
\item the higher the relative level of \DIFdelbegin \DIFdel{i-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal markers }\DIFaddend is, with respect to the remaining sounds, the higher the pleasantness.
\end{itemize}
...
On the contrary, the fact that we observe significant correlations for $L_m$ and $L(E)_m$ and no correlation for $L_m-L_b$ and $L(E)_m$ - $L(E)_b$ for the \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend , shows that this is indeed the absolute level that matters.
...
\item \emph{differentiating \DIFdelbegin \DIFdel{i- }\DIFdelend \DIFaddbegin \DIFadd{ideal }\DIFaddend and \DIFdelbegin \DIFdel{ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes}\DIFaddend }: the semantic features, and the global sound levels ($L$, $L(E)$ et $L(T)$), allow us to differentiate reliably between \DIFdelbegin \DIFdel{i- and ni-scenes}\DIFdelend \DIFaddbegin \DIFadd{ideal and non-ideal scenes}\DIFaddend . The semantic description seems to be more powerful;
...
\begin{itemize}
\item \emph{72 \DIFdelbegin \DIFdel{km-scenes}\DIFdelend \DIFaddbegin \DIFadd{scenes with markers}\DIFaddend }: the 72 scenes originally simulated by the subject of experiment 1.a where the sound classes identified as markers are kept.
...
Thus, only part of the \DIFdelbegin \DIFdel{i-markers and ni-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal markers and non-ideal markers }\DIFaddend are removed from the \DIFdelbegin \DIFdel{rm-scenes}\DIFdelend \DIFaddbegin \DIFadd{scenes without markers}\DIFaddend .
...
The aim here is to validate the hypothesis that the removal of \DIFdelbegin \DIFdel{i-markers and ni-markers have an impact }\DIFdelend \DIFaddbegin \DIFadd{ideal markers and non-ideal markers has a significant effect }\DIFaddend on the perceived pleasantness. To do so, we perform \DIFdelbegin \DIFdel{a variance analysis }\DIFdelend \DIFaddbegin \DIFadd{an analysis of variance (ANOVA)}\DIFaddend . We consider \DIFdelbegin \DIFdel{$\mathcal{A}_{subject}$ }\DIFdelend \DIFaddbegin \DIFadd{the pleasantness rating of each subject on each scene }\DIFaddend as a dependent variable, and as independent variables, the type of environment (\DIFdelbegin \DIFdel{i/ni}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend ) and the presence/absence of markers\DIFdelbegin \DIFdel{(km/rm)}\DIFdelend . As each subject evaluated the whole set of scenes, a two factors repeated measure ANOVA \DIFaddbegin \DIFadd{with interaction }\DIFaddend is used to evaluate whether there exist a significant difference of perceived pleasantness\DIFdelbegin \DIFdel{between km-scenes and rm-scenes}\DIFdelend . The two independent variables are considered as \emph{within-subject} factors. The factors being of only two levels each (type: \DIFdelbegin \DIFdel{i/ni}\DIFdelend \DIFaddbegin \DIFadd{ideal or non-ideal}\DIFaddend ,  marker: \DIFdelbegin \DIFdel{km/rm}\DIFdelend \DIFaddbegin \DIFadd{with or without}\DIFaddend ), the sphericity hypothesis does not need to be checked. \emph{Post hoc} analyses are done using the Tukey-Kramer procedure. %\jfp{l'analyse est inutile a mon avis, il n'y a que 2 niveaux pour chaque facteur de l'ANOVA.} \gl{2 niveaux par facteur, mais 4 niveaux en tout}

%DIF >  $\mathcal{A}_{subject}$
\DIFaddbegin 

\DIFaddend %Tous les tests de significativité sont effectués avec un seuil critique $\alpha=0.05$.
...
Let us consider $\mathcal{A}_{subject}$ for the \DIFdelbegin \DIFdel{km-scenes }\DIFdelend \DIFaddbegin \DIFadd{scenes with markers }\DIFaddend (Figure~\ref{fig:xp2_Asubject}). Close inspection of the answers show that one subject's judgments strongly differ from the others'. This subject evaluated positively close to half of the \DIFdelbegin \DIFdel{ni/km-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes with markers }\DIFaddend (see Annex~\ref{app:xp2}, subject 7), and gave a score above 0 for 58\% of the \DIFdelbegin \DIFdel{ni/km-scenes}\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes with markers}\DIFaddend , contrary to an average of 11\% for the other subjects.
...
In this section, we study the scores given by the subject while listening to the several types of scenes, namely the \DIFdelbegin \DIFdel{i/km-scenes, ni/km-scenes, i/rm-scenes and ni/rm-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes with markers, non-ideal scenes with markers, ideal scenes without markers and non-ideal scenes without markers }\DIFaddend (Figure~\ref{fig:xp2_Ascene}). The repeated measure ANOVA applied to \DIFdelbegin \DIFdel{$\mathcal{A}_{subject}$ }\DIFdelend \DIFaddbegin \DIFadd{the pleasantness rating }\DIFaddend shows a significant effect of the type environment (\DIFdelbegin \DIFdel{i/ni}\DIFdelend \DIFaddbegin \DIFadd{Type}\DIFaddend : $F[1,10]=175$, $p<0.01$), of the presence/absence of markers (\DIFdelbegin \DIFdel{km/rm}\DIFdelend \DIFaddbegin \DIFadd{marker}\DIFaddend : $F[1,10]=7$, $p<0.05$), and of the interaction between those two factors ($F[1,10]=67$, $p<0.01$).

%DIF >   $\mathcal{A}_{subject}$
\DIFaddbegin 

\DIFaddend %The \emph{post hoc} montre, quant à elle, des différences significatives entre tous les groupes d'observations, notamment entre les i/am- et i/sm-scenes ($p<0.05$) et les ni/am- et ni/sm-scenes ($p<0.01$).
...
The significant interaction shows that the type of environment influences the effect of the presence/absence of the markers. Indeed, the average difference between \DIFdelbegin \DIFdel{km-scenes and rm-scenes }\DIFdelend \DIFaddbegin \DIFadd{the scenes with markers and the scenes without markers }\DIFaddend is larger for the \DIFdelbegin \DIFdel{ni-scenes }\DIFdelend \DIFaddbegin \DIFadd{non-ideal scenes }\DIFaddend ($1.1$) than for the \DIFdelbegin \DIFdel{i-scenes }\DIFdelend \DIFaddbegin \DIFadd{ideal scenes }\DIFaddend ($0.5$).

\DIFdelbegin \subsection{\DIFdel{Discussion}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend %DIF > \subsection{Discussion}
...
This strongly confirms that \DIFdelbegin \DIFdel{i-markers }\DIFdelend \DIFaddbegin \DIFadd{ideal markers }\DIFaddend do have a positive impact on the perception of an environment. The fact that their removal \DIFdelbegin \DIFdel{decreases }\DIFdelend \DIFaddbegin \DIFadd{decrease }\DIFaddend $\mathcal{A}_{scene}$ \DIFdelbegin \DIFdel{indicate }\DIFdelend \DIFaddbegin \DIFadd{indicates }\DIFaddend that it should be possible to improve the perceived pleasantness of a given urban area by the addition of sounds commonly considered as pleasant such as \emph{bird calls}. Those conclusions are in line with the positive approach introduced by\DIFdelbegin \DIFdel{Schafer in }\DIFdelend \DIFaddbegin \DIFadd{~}\DIFaddend \cite{schafer1977tuning}.
...
First, the fact that the semantic information (which sound source is present) and compositional information (at which level) are different for ideal and non-ideal scenes leads us to believe that these two types of information characterize the \DIFdelbegin \DIFdel{PE and UE }\DIFdelend \DIFaddbegin \DIFadd{above cited }\DIFaddend concepts.
